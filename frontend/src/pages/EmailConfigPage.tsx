...
interface ISMSConfig {
  ...
  enabled: boolean;
  userId: string;
  password?: string; // Optional in frontend to handle masking
  triggers: {
...
const EmailConfigPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'email' | 'whatsapp' | 'sms' | 'test'>('email');
  ...
  const [selectedWaTrigger, setSelectedWaTrigger] = useState<string | null>(null);
  const [waTriggerEdits, setWaTriggerEdits] = useState<Partial<WhatsAppTrigger>>({});
  ...
  const [selectedSmsTrigger, setSelectedSmsTrigger] = useState<string | null>(null);
  const [smsTriggerEdits, setSmsTriggerEdits] = useState<Partial<ISMSTrigger>>({});
  const [testSmsPhone, setTestSmsPhone] = useState('');
  const [testingSmsTrigger, setTestingSmsTrigger] = useState<string | null>(null);
  ...
  const [bundleResult, setBundleResult] = useState<Record<string, { success: boolean; message: string }> | null>(null);
    ...
    const init = async () => {
      ...
      try {
        ...
        if (projectContext.projectId) {
          console.log('Found projectId in projectContext:', projectContext.projectId);
  ...
  const fetchSMSConfig = async () => {
    ...
    try {
      setSmsLoading(true);
      const token = localStorage.getItem('authToken');
      const response = await axios.get(`${API_BASE_URL}/sms-config/${projectId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const updateWaTriggerField = async (triggerKey: string, field: string, value: any) => {
    if (!projectId || !whatsappConfig) return;
    setWhatsappConfig({
      ...whatsappConfig,
      triggers: {
        ...whatsappConfig.triggers,
        [triggerKey]: { ...whatsappConfig.triggers[triggerKey], [field]: value }
      }
    });
    try {
      const token = localStorage.getItem('authToken');
  ...
  const testWaTrigger = async (triggerKey: string) => {
    ...
    try {
      ...
      setWaTestResult({ success: true, message: response.data.message || 'Test message sent!' });
    } catch (error: any) {
      setWaTestResult({
        success: false,
    ...
    try {
      ...
      if (whatsappConfig) {
        setWhatsappConfig({
          ...
          triggers: {
            ...whatsappConfig.triggers,
            [selectedWaTrigger]: { ...whatsappConfig.triggers[selectedWaTrigger], ...waTriggerEdits }
          }
        });
      }
  ...
  const updateSMSTriggerField = async (triggerKey: string, field: string, value: any) => {
    ...
    try {
      ...
      if (smsConfig) {
        setSmsConfig({
          ...
          triggers: {
            ...smsConfig.triggers,
            [selectedSmsTrigger]: { ...smsConfig.triggers[selectedSmsTrigger], ...smsTriggerEdits }
          }
        });
      }
        ...
        {activeTab === 'email' && config && (
          <div className="space-y-6">
                ...
                <div className="px-6 pb-6 border-t border-gray-100">
                  <div className="space-y-6 mt-4">
                    ...
                    <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      <div>
                        <h4 className="text-lg font-semibold text-gray-900">Email Integration</h4>
                    ...
                    <div className="border-t pt-6">
                      <h4 className="text-lg font-semibold text-gray-900 mb-4">Test Email Configuration</h4>
                      ...
                      {testResult && (
                        <div className={`mt-4 p-4 rounded-lg flex items-center gap-3 ${testResult.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                          {testResult.success ? <CheckCircleIcon className="h-6 w-6" /> : <XCircleIcon className="h-6 w-6" />}
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <p className="text-sm text-gray-600 mb-6">Configure when and how email notifications are sent</p>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`px-3 py-1 text-xs font-medium rounded-full ${trigger.enabled ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>
                        ...
                        <label className="relative inline-flex items-center cursor-pointer">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              updatedConfig.triggers[key].enabled = e.target.checked;
                ...
                <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                  <div className="p-6">
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <button
                        onClick={() => { setSelectedTrigger(null); setTriggerEdits({}); }}
                    ...
                    <div className="space-y-6">
                      <div>
                        ...
                        <input
                          type="text"
                          value={triggerEdits.subject ?? config.triggers[selectedTrigger].subject}
                      ...
                      <div>
                        ...
                        <textarea
                          value={triggerEdits.body ?? config.triggers[selectedTrigger].body}
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {TRIGGER_VARIABLES[selectedTrigger]?.map((v) => (
                            <div key={v.variable} className="text-sm">
                              <code className="bg-blue-100 px-2 py-1 rounded text-blue-800">{v.variable}</code>
                      ...
                      <div>
                        ...
                        <select
                          value={triggerEdits.recipients ?? config.triggers[selectedTrigger].recipients}
            ...
            <div className="space-y-6">
              {whatsappLoading ? (
                ...
                <>
                  ...
                  <div className="bg-white rounded-lg shadow-md">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold text-gray-900">WhatsApp API Settings</h3>
                          <p className="text-sm text-gray-500">
                            {whatsappConfig.apiBaseUrl ? 'Configured' : 'Not configured'}
                          </p>
                        </div>
                      </div>
                      <svg
                    ...
                    {showWaApiSettings && (
                      <div className="px-6 pb-6 border-t border-gray-100">
                        ...
                        <div className="flex items-center justify-between mb-8 mt-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                          ...
                          <div className="flex items-center gap-3">
                            <span className={`text-sm font-medium ${whatsappConfig.enabled ? 'text-green-600' : 'text-gray-500'}`}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setWhatsappConfig({ ...whatsappConfig, apiBaseUrl: e.target.value })}
                            ...
                            <p className="mt-1 text-xs text-gray-500 break-all">
                              Format: {whatsappConfig.apiBaseUrl || 'https://...'}/{'{numberId}'}/messages
                            </p>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <button
                                type="button"
                                onClick={() => setShowWaAccessToken(!showWaAccessToken)}
                                className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1"
                              >
                                {showWaAccessToken ? <EyeSlashIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                              </button>
                            </div>
                          </div>
                        </div>
                  ...
                  {waTestResult && (
                    <div className={`p-4 rounded-lg flex items-center gap-3 ${waTestResult.success ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                      {waTestResult.success ? <CheckCircleIcon className="h-5 w-5" /> : <XCircleIcon className="h-5 w-5" />}
                      <span>{waTestResult.message}</span>
                      <button onClick={() => setWaTestResult(null)} className="ml-auto text-sm underline">Dismiss</button>
                    </div>
                  )}
                  ...
                  <div className="bg-white rounded-lg shadow-md">
                    <div className="px-6 py-4 border-b border-gray-100">
                      <h3 className="text-lg font-semibold text-gray-900">WhatsApp Triggers</h3>
                      <p className="text-sm text-gray-500">Configure Number ID for each message trigger</p>
                    </div>

                    <div className="divide-y divide-gray-100">
                      {Object.entries(whatsappConfig.triggers).map(([key, trigger]) => (
                              ...
                              <div className="flex flex-wrap items-end gap-3">
                                <div className="flex-1 min-w-[200px]">
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => updateWaTriggerField(key, 'numberId', e.target.value)}
                                ...
                                <div className="w-24">
                                  ...
                                  <select
                                    ...
                                    onChange={(e) => updateWaTriggerField(key, 'templateLanguage', e.target.value)}
                                ...
                                <div className="flex items-end gap-2">
                                  ...
                                  <button
                                    ...
                                    disabled={testingWaTrigger === key || !trigger.numberId}
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <label className="relative inline-flex items-center cursor-pointer">
                                <input
                                  type="checkbox"
                                  checked={trigger.enabled}
                                  onChange={(e) => updateWaTriggerField(key, 'enabled', e.target.checked)}
                                  className="sr-only peer"
                                />
                                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                              </label>
          ...
          selectedWaTrigger && whatsappConfig && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div className="px-6 py-4 border-b border-gray-100">
                  <h2 className="text-xl font-bold text-gray-900">
                    Edit WhatsApp Trigger: {whatsappConfig.triggers[selectedWaTrigger]?.name}
                  </h2>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => setWaTriggerEdits({ ...waTriggerEdits, enabled: e.target.checked })}
                      className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                    />
                    <label htmlFor="waTriggerEnabled" className="text-sm font-medium text-gray-700">
                      Enable this trigger
                    </label>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setWaTriggerEdits({ ...waTriggerEdits, numberId: e.target.value })}
                      placeholder="e.g., 952415067947914"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setWaTriggerEdits({ ...waTriggerEdits, templateLanguage: e.target.value })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    >
                      <option value="en">English</option>
                    ...
                    <p className="mt-1 text-xs text-gray-500">
                      The language of the WhatsApp template. Must match the language your template was approved for.
                    </p>
                  </div>
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-blue-700 mb-3">
                          These variables are sent as template parameters ({'{'}{'{'}1{'}'}{'}'}, {'{'}{'{'}2{'}'}{'}'}, etc.) when the message is triggered.
                        </p>
                        <div className="space-y-2">
                          {selectedWaTrigger === 'accountCreated' && (
                            <>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{1}}'}</code>
                                <span className="text-blue-700">- Student's full name</span>
                              </div>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{2}}'}</code>
                                <span className="text-blue-700">- Student's email</span>
                              </div>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{3}}'}</code>
                          ...
                          {selectedWaTrigger === 'passwordReset' && (
                            <>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{1}}'}</code>
                                <span className="text-blue-700">- User's name</span>
                              </div>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{2}}'}</code>
                                <span className="text-blue-700">- Reset link</span>
                              </div>
                            </>
                          )}
                          {selectedWaTrigger === 'studentOTP' && (
                            <div className="flex items-center gap-2 text-sm">
                              <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{1}}'}</code>
                              <span className="text-blue-700">- OTP code (6 digits)</span>
                            </div>
                          )}
                          {(selectedWaTrigger.includes('ticketCreated') || selectedWaTrigger === 'ticketStatusChanged' || selectedWaTrigger === 'ticketClosed') && (
                            <>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{1}}'}</code>
                                <span className="text-blue-700">- Recipient's name</span>
                              </div>
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{2}}'}</code>
                              ...
                              <div className="flex items-center gap-2 text-sm">
                                <code className="bg-blue-100 px-2 py-0.5 rounded text-blue-800">{'{{3}}'}</code>
                                <span className="text-blue-700">- Ticket subject / Status</span>
                              </div>
                            </>
                          )}
                          {!['accountCreated', 'passwordReset', 'studentOTP'].includes(selectedWaTrigger) && !selectedWaTrigger.includes('ticketCreated') && selectedWaTrigger !== 'ticketStatusChanged' && selectedWaTrigger !== 'ticketClosed' && (
                            <p className="text-sm text-blue-700 italic">
                              Variables depend on the trigger type. Contact admin for specific template requirements.
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setWaTriggerEdits({ ...waTriggerEdits, recipients: e.target.value as any })}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                    >
                      <option value="student">Student Only</option>
                      <option value="agent">Agent Only</option>
                ...
                <>
                  ...
                  <div className="bg-white rounded-lg shadow-md">
                   <div className="px-6 py-4 flex items-center justify-between">
                     <div>
                       <h3 className="text-lg font-semibold text-gray-900">Gupshup SMS Settings</h3>
                       <p className="text-sm text-gray-500">Configure your Gupshup Enterprise Gateway credentials</p>
                     </div>
                   </div>
                    ...
                    <div className="px-6 pb-6 border-t border-gray-100">
                      ...
                      <div className="flex items-center justify-between mb-8 mt-6 bg-gray-50 p-4 rounded-lg border border-gray-100">
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`text-sm font-medium ${smsConfig.enabled ? 'text-green-600' : 'text-gray-500'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setSmsConfig({ ...smsConfig, userId: e.target.value })}
                            placeholder="Enteprise Account ID"
                          ...
                          <label className="block text-sm font-medium text-gray-700 mb-1">
                           ...
                           <div className="relative">
                            <input
                              ...
                              placeholder={smsConfig.password ? "********" : "Enter Password"}
                            ...
                            <button
                              ...
                              className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 p-1"
                            >
                              {showSmsPassword ? <EyeSlashIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                            </button>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="bg-white rounded-lg shadow-md">
                    <div className="px-6 py-4 border-b border-gray-100">
                      <h3 className="text-lg font-semibold text-gray-900">SMS Triggers</h3>
                      <p className="text-sm text-gray-500">Configure templates for each trigger</p>
                    </div>

                    <div className="space-y-4 p-6">
                      {Object.entries(smsConfig.triggers).map(([key, trigger]) => (
                        <div key={key} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                          <div className="flex items-center justify-between gap-4">
                            <div className="flex-1">
                              <div className="flex items-center gap-3 mb-2">
                              ...
                              <div className="mt-3 flex items-center gap-2">
                                <input
                                  type="text"
                                  value={testingSmsTrigger === key ? testSmsPhone : (testSmsPhone || '')} // Shared state issue if multiple inputs. Better to have one shared input or local var. 
                                ...
                                <button
                                  ...
                                  disabled={testingSmsTrigger === key || !testSmsPhone}
                            ...
                            <div className="flex flex-col items-end gap-2">
                              <label className="relative inline-flex items-center cursor-pointer">
                                <input
                                  type="checkbox"
                                  checked={trigger.enabled}
                                  onChange={(e) => updateSMSTriggerField(key, 'enabled', e.target.checked)}
                                  className="sr-only peer"
                                />
                                <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-green-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                              </label>
          ...
          selectedSmsTrigger && smsConfig && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
                <div className="px-6 py-4 border-b border-gray-100">
                  <h2 className="text-xl font-bold text-gray-900">
                    Edit SMS Trigger: {smsConfig.triggers[selectedSmsTrigger]?.name}
                  </h2>
                </div>
                ...
                <div className="p-6 space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      onChange={(e) => setSmsTriggerEdits({ ...smsTriggerEdits, enabled: e.target.checked })}
                      className="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300 rounded"
                    />
                    <label htmlFor="smsTriggerEnabled" className="text-sm font-medium text-gray-700">
                      Enable this trigger
                    </label>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) => setSmsTriggerEdits({ ...smsTriggerEdits, template: e.target.value })}
                      rows={5}
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent font-mono text-sm"
                      placeholder="Enter your SMS template content here..."
                    />
                    <p className="mt-1 text-xs text-gray-500">
                      Supports variables like {'{{1}}'}, {'{{2}}'}, etc. or named variables {'{{otp}}'} depending on trigger context.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setSmsTriggerEdits({ ...smsTriggerEdits, recipients: e.target.value as any })}
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="space-y-1 text-sm text-blue-800">
                           ...
                           {selectedSmsTrigger === 'studentOTP' && <p><code>{`{{otp}}`}</code> - The 6-digit OTP code</p>}
                           {selectedSmsTrigger === 'ticketCreatedStudent' && <p><code>{`{{ticketNumber}}`}</code>, <code>{`{{ticketTitle}}`}</code></p>}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg shadow-md">
              <div className="px-6 py-6">
                <h3 className="text-xl font-semibold text-gray-900 mb-2">Test Notifications (Email / WhatsApp / SMS)</h3>
                <p className="text-sm text-gray-600 mb-6">Send test messages to verify your notification configurations across all channels</p>

                {/* Channel Selection */}
                <div className="mb-6">
                  <label className="block text-sm font-medium text-gray-700 mb-3">Select Channels to Test</label>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <label className="flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Test email notifications</p>
                      </div>
                    </label>
                    <label className="flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <input
                        ...
                        onChange={(e) => setBundleSendWhatsApp(e.target.checked)}
                        className="h-5 w-5 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                      />
                      <div>
                        <span className="text-sm font-medium text-gray-900">WhatsApp</span>
                        <p className="text-xs text-gray-500">Test WhatsApp templates</p>
                      </div>
                    </label>
                    <label className="flex items-center gap-3 p-4 border-2 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors">
                      <input
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">Test SMS messages</p>
                      </div>
                    </label>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Test Email Address
                      {bundleSendEmail && <span className="text-red-500 ml-1">*</span>}
                    </label>
                    <input
                      type="email"
                    ...
                    />
                    <p className="mt-1 text-xs text-gray-500">Required if Email is selected</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Test Phone Number (with country code)
                      {(bundleSendWhatsApp || bundleSendSMS) && <span className="text-red-500 ml-1">*</span>}
                    </label>
                    <input
                      type="tel"
                      ...
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent disabled:bg-gray-100 disabled:cursor-not-allowed"
                    />
                    <p className="mt-1 text-xs text-gray-500">Required if WhatsApp or SMS is selected</p>
                  </div>
                </div>
                ...
                <div className="flex justify-end border-t pt-6">
                  <button
                    ...
                    disabled={bundleTesting || (!bundleSendEmail && !bundleSendWhatsApp && !bundleSendSMS)}
                  ...
                  <div className="mt-6 space-y-3 border-t pt-6">
                    ...
                    {Object.entries(bundleResult).map(([channel, result]) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <span className="font-semibold capitalize text-sm">{channel}</span>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h4 className="text-sm font-semibold text-blue-900 mb-2">Testing Guidelines</h4>
                  <ul className="text-sm text-blue-800 space-y-1">
                    <li>• Ensure each channel is properly configured before testing</li>
                    <li>• Integration must be enabled for the channel to send test messages</li>
                    <li>• For WhatsApp, at least one trigger must have numberId and templateName configured</li>
                    <li>• Test messages use sample content and may not reflect actual trigger templates</li>
                    <li>• Check spam/junk folders if you don't receive the email test</li>
                  </ul>
                </div>
              </div>
            </div>
