...
const SLARulesPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    dashboardCategory: "" as "" | "high" | "medium" | "low",
  });
  ...
  const fetchPriorities = async () => {
    try {
      ...
      const token = localStorage.getItem("authToken");
      console.log(
        "Fetching priorities with token:",
    ...
    if (confirm(`Are you sure you want to delete "${priority.name}"?`)) {
      try {
        ...
        const response = await fetch(
          `${API_CONFIG.API_URL}/sla-rules/${priority._id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleToggleStatus = async (priority: Priority) => {
    try {
      ...
      const response = await fetch(
        `${API_CONFIG.API_URL}/sla-rules/${priority._id}/toggle-status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleSavePriority = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const priorityData = {
        ...
        dashboardCategory: formData.dashboardCategory || undefined,
      };
      ...
      console.error(
        `Error ${editingPriority ? "updating" : "creating"} priority:`,
        error,
      );
  ...
  const handleEditPriority = (priority: Priority) => {
    ...
    const extractedProjectIds =
      priority.projectIds && Array.isArray(priority.projectIds)
    ...
    setFormData({
      ...
      dashboardCategory:
        (priority.dashboardCategory as "" | "high" | "medium" | "low") || "",
    });
  ...
  return (
    <DashboardLayout>
      <div style={{ padding: "32px", maxWidth: "1400px", margin: "0 auto" }}>
          ...
          <>
            ...
            {priorities.length === 0 ? (
              ...
              >
                ...
                >
                  Create your first priority to define response and resolution
                  times
                </p>
                <button
                  onClick={() => setShowCreateModal(true)}
                        ...
                        >
                          ...
                          <span
                            style={{
                              ...
                              fontSize: "12px",
                              fontWeight: 500,
                              backgroundColor: priority.isActive
                                ? "#dcfce7"
                                : "#fee2e2",
                      ...
                      >
                        ...
                        >
                          {priority.responseTime
                            ? formatTime(priority.responseTime)
                            : "Not set"}
                        </div>
                      </div>
                      <div
                      ...
                      >
                        ...
                        >
                          {priority.resolutionTime
                            ? formatTime(priority.resolutionTime)
                            : "Not set"}
                        </div>
                      </div>
                    </div>
                ...
                >
                  ...
                  <form onSubmit={handleSavePriority}>
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      ...
                      >
                        Map this priority to a dashboard group. The dashboard
                        card will show this priority's name but count tickets
                        using the mapped category.
                      </p>
                      <select
                        required
                        value={formData.dashboardCategory}
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            dashboardCategory: e.target.value as
                              | ""
                              | "high"
                              | "medium"
                              | "low",
                      ...
                      >
                        ...
                        <option value="high">
                          🔴 High (e.g., Urgent, Critical)
                        </option>
                        <option value="medium">
                          🟡 Medium (e.g., Normal, Standard)
                        </option>
                        <option value="low">
                          🟢 Low (e.g., Routine, Minor)
                        </option>
                      </select>
                    </div>
                      ...
                      <div style={{ display: "flex", gap: "12px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              responseTimeUnit: e.target.value as any,
                            })
                          }
                          style={{
                            width: "120px",
                      ...
                      <div style={{ display: "flex", gap: "12px" }}>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              resolutionTimeUnit: e.target.value as any,
                            })
                          }
                          style={{
                            width: "120px",
                        ...
                        {projects.length === 0 ? (
                          ...
                          projects.map((project) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={formData.projectIds.includes(
                                  project._id,
                                )}
                                onChange={(e) => {
                                  if (e.target.checked) {
        ...
        {activeTab === "escalation" && <EscalationMatrixContent />}

        {/* Working Calendar Tab Content */}
        {activeTab === "working" && <WorkingCalendarContent />}
      </div>
    </DashboardLayout>
  );
};
