...
const AgentTicketsContent = ({
  ...
  const fetchTags = async () => {
    try {
      const token = localStorage.getItem("authToken");
      const response = await axios.get(`${API_CONFIG.API_URL}/tickets/tags`, {
        headers: { Authorization: `Bearer ${token}` },
        params: { projectId },
      });
  ...
  const fetchStatuses = async () => {
    try {
      ...
      const mapped =
        response.data.data?.map((status: any) => ({
          name: status.name,
          value: (status.code
            ? String(status.code)
            : String(status.name)
  ...
  const handleBulkStatusChange = async () => {
    ...
    try {
      ...
      alert(
        `Successfully updated ${selectedTickets.size} tickets to ${bulkStatus}`,
      );
      setSelectedTickets(new Set());
      setShowBulkActions(false);
      setBulkStatus("");
  ...
  const handleBulkClose = async () => {
    ...
    if (
      !confirm(
        `Are you sure you want to close ${selectedTickets.size} tickets?`,
      )
    )
      return;
  ...
  const filteredTickets = tickets.filter((ticket) => {
    ...
    if (filterStatus.length > 0) {
      ...
      const normalizedFilters = filterStatus.map((f) =>
  ...
  const getTicketCountsByTab = () => {
    return {
      online: tickets.filter((t) => {
        ...
        return (
          source === "online" && status !== "resolved" && status !== "closed"
        );
      }).length,
      offline: tickets.filter((t) => {
        ...
        const status = String(t.status || "").toLowerCase();
        return (
          source === "offline" && status !== "resolved" && status !== "closed"
        );
      }).length,
    };
  };
  ...
  const showOfflineTab =
    submissionMode === "offline" || submissionMode === "both";

  const getStatusColor = (status: string) => {
    const colors: Record<string, string> = {
      open: "background: #fef3c7; color: #92400e; borderColor: #fbbf24",
      "in-progress":
        "background: #dbeafe; color: #1e40af; borderColor: #3b82f6",
        ...
        >
          ...
          <div style={{ flex: "1", minWidth: "150px" }}>
            ...
            <select
              ...
              onChange={(e) =>
                setFilterCategory(e.target.value ? [e.target.value] : [])
              }
              style={{
                width: "100%",
                padding: "10px 12px",
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          {filteredTickets.map((ticket) => (
            <div
              key={ticket._id}
              style={{
                background: ticket.hasNewReply ? "#FFFBEB" : "white",
                padding: "20px",
                borderRadius: "12px",
                border: selectedTickets.has(ticket._id)
                  ? "2px solid #3b82f6"
                  : ticket.hasNewReply
                    ? "1.5px solid #F59E0B"
                    : "1px solid #e5e7eb",
                borderLeft:
                  ticket.hasNewReply && !selectedTickets.has(ticket._id)
                    ? "4px solid #F59E0B"
                    : undefined,
                boxShadow: ticket.hasNewReply
                  ? "0 1px 6px rgba(245,158,11,0.2)"
            ...
            >
              <div style={{ display: "flex", gap: "12px" }}>
                ...
                <div
                  onClick={() =>
                    navigate(`/${customUrlPath}/portal/ticket/${ticket._id}`)
                  }
...
const ProjectPortalDashboard = () => {
  ...
  const getModuleAccessFromPermissions = (permissions: string[]) => {
    ...
    const hasDashboardPermission = permissions.some((p) =>
      p.startsWith("DASHBOARD_"),
    );
    const hasTicketPermission = permissions.some((p) =>
      p.startsWith("TICKET_"),
    );
    const hasKnowledgeBasePermission = permissions.some((p) =>
      p.startsWith("KB_"),
    ); // Fixed: KB_ not KNOWLEDGE_BASE_
