...
const ApprovalWorkflows: React.FC = () => {
  ...
  const fetchWorkflows = async (projId: string) => {
    try {
      const response = await axios.get(
        `${API_BASE}/approvals/project/${projId}`,
      );
      ...
      try {
        ...
        if (parts.length === 3) {
          ...
          if (payload.permissions && Array.isArray(payload.permissions)) {
            const hasProjectViewAll =
              payload.permissions.includes("PROJECT_VIEW_ALL");
            if (hasProjectViewAll) {
              console.log(
                "✓ User has PROJECT_VIEW_ALL permission - loading all projects",
              );
              fetchProjects();
              return;
            } else {
  ...
  const handleEditWorkflow = (workflow: any) => {
    ...
    const categoryName =
      typeof workflow.categoryId === "object" ? workflow.categoryId.name : "";
    setCategorySearch(categoryName);

    // Convert workflow levels to LevelForm format
    const workflowLevels = (workflow.levels || []).map(
      (level: any, index: number) => ({
        tempId: `level-${Date.now()}-${index}`,
        role:
          Array.isArray(level.roles) && level.roles.length > 0
            ? typeof level.roles[0] === "object"
              ? level.roles[0]._id
              : level.roles[0]
            : "",
  ...
  const handleLevelRoleChange = (tempId: string, value: string) => {
    setLevels((prev) =>
      prev.map((level) =>
        level.tempId === tempId ? { ...level, role: value } : level,
      ),
    );
  };
  ...
  const handleRemoveLevel = (tempId: string) => {
    setLevels((prev) =>
      prev.length === 1
        ? prev
        : prev.filter((level) => level.tempId !== tempId),
    );
  };
  ...
  const filteredWorkflows = useMemo(() => {
    ...
    return workflows.filter((workflow) => {
      ...
      const categoryName =
        typeof workflow.categoryId === "object"
          ? workflow.categoryId?.name?.toLowerCase()
          : "";
      const requestTypeName =
        typeof workflow.requestTypeId === "object"
          ? workflow.requestTypeId?.name?.toLowerCase()
          : "";
      return (
        name.includes(term) ||
        categoryName?.includes(term) ||
        requestTypeName?.includes(term)
      );
    });
  ...
  const resolveCategoryName = (workflow: any) => {
    ...
    if (typeof workflow.categoryId === "object")
      return workflow.categoryId.name || "—";
    const fallback = masters.categories.find(
      (c) => c._id === workflow.categoryId,
    );
          ...
          <>
            ...
            {hasPermission("PROJECT_VIEW_ALL") && (
              ...
              >
                ...
                >
                  ...
                  >
                    Select Project to Manage Approval Workflows
                  </label>
                </div>
                <p
                  style={{
            ...
            {!projectId && hasPermission("PROJECT_VIEW_ALL") ? (
              ...
              >
                ...
                >
                  Please select a project from the dropdown above to manage its
                  approval workflows
                </p>
              </div>
            ) : (
                          ...
                          {filteredWorkflows.map((workflow, index) => (
                            ...
                            >
                              ...
                              <td style={{ padding: "12px 16px" }}>
                                ...
                                >
                                  {workflow.approvalLogic === "parallel"
                                    ? "Parallel"
                                    : "Sequential"}{" "}
                                  • {(workflow.levels || []).length || 0} levels
                                </div>
                              </td>
                              <td style={{ padding: "12px 16px" }}>
                                {workflow.autoApprove
                                  ? renderBadge("Enabled", "green")
                                  : renderBadge("Disabled", "gray")}
                              </td>
                              <td style={{ padding: "12px 16px" }}>
                                <label
                                  style={{
                                    ...
                                    onClick={async () => {
                                      if (
                                        ...
                                        try {
                                          ...
                                          await axios.delete(
                                            `${API_BASE}/approvals/${workflow._id}`,
                                            {
                                              headers: {
                                                Authorization: token
                                                  ? `Bearer ${token}`
                    ...
                    >
                      ...
                      >
                        <div>
                          ...
                          >
                            Define multi-level approvals driven by roles
                          </p>
                        </div>
                        <button
                            ...
                            <div>
                              ...
                              <select
                                ...
                                onChange={(event) =>
                                  setForm((prev) => ({
                                    ...prev,
                                    approvalLogic: event.target.value,
                                  }))
                                }
                                style={{
                                  width: "100%",
                                ...
                                {levels.map((level, index) => (
                                  ...
                                  >
                                    ...
                                    >
                                      ...
                                      <button
                                        onClick={() =>
                                          handleRemoveLevel(level.tempId)
                                        }
                                        style={{
                                          background: "transparent",
                                          border: "none",
                  ...
                  >
                    ...
                    >
                      ...
                      <div style={{ padding: "24px" }}>
                        ...
                        {/* Approval Hierarchy */}
                        {viewingWorkflow.levels &&
                          viewingWorkflow.levels.length > 0 && (
                            <div style={{ marginBottom: "20px" }}>
                              <label
                                style={{
                                  display: "block",
                              ...
                              >
                                Approval Hierarchy (
                                {viewingWorkflow.levels.length} Level
                                {viewingWorkflow.levels.length !== 1 ? "s" : ""}
                                )
                              </label>
                              <div
                                style={{
                                  ...
                                  (level: any, index: number) => (
                                    <div
                                      ...
                                      style={{
                                        ...
                                        borderBottom:
                                          index <
                                          viewingWorkflow.levels.length - 1
                                            ? "1px solid #E5E7EB"
                                            : "none",
                                      }}
                                    >
                        ...
                        {viewingWorkflow.autoApprove && (
                          ...
                          >
                            <strong>Auto-approval enabled:</strong> Requests
                            will be automatically approved if no action is
                            taken.
                          </div>
                        )}
                      </div>
