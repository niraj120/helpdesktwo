...
const FeedbackFormManagement: React.FC = () => {
  ...
  const [selectedFormForBuilder, setSelectedFormForBuilder] = useState<string | null>(null);
  ...
  const handleEdit = (form: FeedbackForm) => {
    ...
    setFormData({
      ...
      triggers: form.triggers || [{ type: 'ticket_closed', enabled: true, conditions: {} }],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.triggers || formData.triggers.length === 0) {
      alert('Please select at least one trigger for when to send this feedback form');
      return;
    }
    ...
    try {
      ...
      alert(error.response?.data?.message || error.message || 'Failed to save feedback form');
    }
  };
    ...
    <DashboardLayout>
        ...
        {selectedProjectId && (
          <>
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Forms Library</h2>
                <p className="text-gray-600 text-sm mt-1">Manage all feedback forms for the selected project</p>
              </div>
              <button
      ...
      {forms.length === 0 ? (
        <div className="text-center py-16 bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl border-2 border-dashed border-gray-300">
          ...
          <h3 className="text-xl font-semibold text-gray-900 mb-2">No Feedback Forms Yet</h3>
          <p className="text-gray-600 mb-6 max-w-md mx-auto">Get started by creating your first feedback form to collect valuable insights from students</p>
          <button
            onClick={handleCreate}
              ...
              <div className="bg-gradient-to-r from-blue-500 to-indigo-600 p-4">
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    {form.description && (
                      <p className="text-sm text-blue-100 line-clamp-2">{form.description}</p>
                    )}
                  </div>
                  {form.isActive ? (
              ...
              <div className="p-5">
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                    ...
                    <p className="text-lg font-bold text-blue-900">{form.questions?.length || 0}</p>
                  </div>
                  <div className="bg-purple-50 rounded-lg p-3 border border-purple-100">
                    <p className="text-xs text-purple-600 font-medium mb-1">Email</p>
                    <p className="text-sm font-semibold text-purple-900">{form.settings.sendEmailNotification ? 'Enabled' : 'Disabled'}</p>
                  </div>
                </div>
                {form.settings.emailDelay ? (
                  <div className="bg-amber-50 rounded-lg p-2 mb-4 border border-amber-100">
                    <p className="text-xs text-amber-700">
                      <span className="font-semibold">Delay:</span> {form.settings.emailDelay} minutes after ticket closure
                    </p>
                  </div>
                ) : null}
            ...
            <div className="p-6">
              ...
              {!editingForm && (
                <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-4 rounded">
                  <div className="flex items-start">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-blue-900 mb-1">Form Creation Steps</p>
                      <ol className="text-sm text-blue-800 space-y-1 list-decimal list-inside">
                        <li>Fill in the basic form details below</li>
                        <li>Click "Create Form" to save</li>
                        <li>Click the <span className="font-semibold">"Design Form"</span> button to add questions (rating, text, checkboxes, etc.)</li>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                      ...
                      <>
                        <div>
                          ...
                          <p className="text-xs text-gray-500 mt-1">
                            Delay before sending feedback email (0 = send immediately)
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Use {'{ticketNumber}'}, {'{studentName}'} as placeholders
                          </p>
                        </div>
                        ...
                        <div>
                          ...
                          />
                          <p className="text-xs text-gray-500 mt-1">
                            Use {'{ticketNumber}'}, {'{studentName}'}, {'{feedbackLink}'} as placeholders
                          </p>
                        </div>
                      </>
                    )}
                ...
                <div className="border-t pt-4">
                  ...
                  <p className="text-sm text-gray-600 mb-3">Select when to send this feedback form to users</p>
                  
                  <div className="space-y-2">
                    {[
                      { value: 'ticket_created', label: 'When Ticket is Created', desc: 'Send immediately after ticket creation' },
                      { value: 'ticket_closed', label: 'When Ticket is Closed/Resolved', desc: 'Send when ticket status is marked as closed' },
                      { value: 'ticket_status_changed', label: 'On Specific Status Change', desc: 'Send when ticket moves to selected statuses' }
                    ].map((trigger) => (
                      <label key={trigger.value} className="flex items-start p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
                        <input
                          type="checkbox"
                          checked={formData.triggers?.some(t => t.type === trigger.value && t.enabled)}
                          onChange={(e) => {
                            const newTriggers = e.target.checked
                              ? [...(formData.triggers || []), { type: trigger.value as any, enabled: true, conditions: {} }]
                              : formData.triggers?.filter(t => t.type !== trigger.value) || [];
                ...
                {!editingForm && (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-3 mb-4">
                    <p className="text-sm text-amber-800">
                      <span className="font-semibold">💡 Note:</span> After creating the form, use the <span className="font-semibold">"Design Form"</span> button to add questions like ratings, text fields, multiple choice, etc.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3 pt-4">
                  ...
                  >
                    {editingForm ? 'Update Form' : 'Create Form & Add Questions'}
                  </button>
                </div>
              </form>
            </div>
