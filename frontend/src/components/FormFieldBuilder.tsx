...
import { FormFieldSchema, FieldCondition, ConditionOperator, RequiredMode } from '../utils/conditionEngine';
import FormRenderer from './FormRenderer';
import HierarchyCategorySelector, { CategoryHierarchyValue } from './HierarchyCategorySelector';
...
const RuleBuilder: React.FC<RuleBuilderProps> = ({
  ...
  const add = () =>
    onConditionsChange([
      ...conditions,
      { triggerField: triggerFieldNames[0] || '', operator: 'equals', value: '' },
    ]);
  ...
  const remove = (idx: number) => onConditionsChange(conditions.filter((_, i) => i !== idx));
  ...
  return (
    <div>
      ...
      {conditions.length === 0 ? (
        <div style={{ fontSize: '13px', color: '#9ca3af', padding: '10px 14px', background: '#f9fafb', borderRadius: '6px' }}>
          No conditions — field is {action === 'hide' ? 'always visible' : action === 'show' ? 'always visible' : 'evaluated at submit time'}.
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '8px' }}>
          {conditions.map((cond, idx) => (
            <div key={idx} style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
              <span style={{ fontSize: '11px', color: '#6b7280', width: '34px', textAlign: 'center', flexShrink: 0 }}>
                {idx === 0 ? 'When' : 'AND'}
              </span>
              <select
              ...
              >
                {triggerFieldNames.map(n => <option key={n} value={n}>{n}</option>)}
              </select>
              <select
                value={cond.operator}
              ...
              <button
                type="button"
                onClick={() => remove(idx)}
                style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#ef4444', padding: '4px', flexShrink: 0 }}
...
const FieldConfigPanel: React.FC<FieldConfigPanelProps> = ({ field, onUpdate, triggerFieldNames }) => {
  const [tab, setTab] = useState<'basic' | 'conditions' | 'required-if'>('basic');
  ...
  const tabBtn = (id: 'basic' | 'conditions' | 'required-if', label: React.ReactNode): React.ReactNode => (
    <button
      key={id}
      type="button"
      onClick={() => setTab(id)}
  ...
  return (
    <div style={{ marginTop: '12px', padding: '14px', background: '#f9fafb', borderRadius: '8px', border: '1px solid #e5e7eb' }} onKeyDown={e => e.stopPropagation()}>
      {/* Tab bar */}
      <div style={{ display: 'flex', gap: '2px', borderBottom: '1px solid #e5e7eb', marginBottom: '14px' }}>
        {tabBtn('basic', 'Basic')}
        {tabBtn(
          'conditions',
          <>
            Conditions{' '}
            {condCount > 0 && (
              <span style={{ marginLeft: '4px', fontSize: '11px', background: '#dbeafe', color: '#1d4ed8', borderRadius: '10px', padding: '1px 6px' }}>
                {condCount}
              </span>
            )}
          </>,
        )}
        {field.requiredMode === 'conditional' && tabBtn('required-if', 'Required If')}
      </div>
      ...
      {tab === 'basic' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '4px' }}>
                Field Name <span style={{ color: '#9ca3af', fontWeight: '400' }}>(key)</span>
              </label>
              <input
                type="text"
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '4px' }}>Display Label</label>
              <input
                type="text"
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '4px' }}>Field Type</label>
              <select
                value={field.fieldType}
                onChange={e =>
                  onUpdate({
                    fieldType: e.target.value,
                    options: NEEDS_OPTIONS.has(e.target.value) ? (field.options || []) : [],
                  })
                }
                style={inputSty}
              >
            ...
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '4px' }}>Placeholder</label>
              <input
                type="text"
          ...
          {/* Required mode */}
          <div>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '6px' }}>Required</label>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              {(['always', 'conditional', 'optional'] as RequiredMode[]).map(mode => (
                <button
                  key={mode}
                  type="button"
                  onClick={() => onUpdate({ requiredMode: mode, required: mode === 'always' })}
                  style={{
                    padding: '5px 12px',
                    borderRadius: '6px',
                    border: `1px solid ${field.requiredMode === mode ? '#3b82f6' : '#d1d5db'}`,
                    background: field.requiredMode === mode ? '#dbeafe' : 'white',
                    color: field.requiredMode === mode ? '#1d4ed8' : '#4b5563',
                    fontSize: '12px',
                    cursor: 'pointer',
                    fontWeight: field.requiredMode === mode ? '600' : '400',
                  }}
                >
                  {mode === 'always' ? 'Always required' : mode === 'conditional' ? 'Required if…' : 'Optional'}
                </button>
              ))}
            </div>
            {field.requiredMode === 'conditional' && (
              <p style={{ margin: '6px 0 0', fontSize: '12px', color: '#6b7280' }}>
                Open the <strong>Required If</strong> tab to define when this field is required.
              </p>
            )}
          </div>
          ...
          {NEEDS_OPTIONS.has(field.fieldType) && (
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '4px' }}>
                Options <span style={{ color: '#9ca3af', fontWeight: '400' }}>(one per line)</span>
              </label>
              <textarea
                value={(field.options || []).join('\n')}
                onChange={e => {
                  const val = e.target.value;
                  onUpdate({ options: val.split('\n').map(s => s.trim()).filter(Boolean) });
                }}
                rows={4}
          ...
          {field.fieldType === 'file' && (
            <div style={{ padding: '12px', background: '#fef9c3', border: '1px solid #fde047', borderRadius: '6px' }}>
              <div style={{ marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {['.pdf', '.doc', '.docx', '.xls', '.xlsx', '.jpg', '.jpeg', '.png', '.gif', '.txt', '.zip'].map(ext => {
                    const checked = (field.allowedFileTypes || []).includes(ext);
                    return (
                      <label
                        key={ext}
                        style={{
                      ...
                      >
                        <input
                          ...
                          onChange={e => {
                            ...
                            onUpdate({ allowedFileTypes: e.target.checked ? [...cur, ext] : cur.filter(t => t !== ext) });
                          }}
              ...
              <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={e => onUpdate({ maxFileSizeMB: parseInt(e.target.value) || 5 })}
                    style={{ width: '80px', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' }}
                  />
                </div>
                <label style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', fontSize: '13px', color: '#78350f' }}>
                  <input
                    type="checkbox"
          ...
          <div>
            <label style={{ display: 'block', fontSize: '12px', fontWeight: '500', color: '#4b5563', marginBottom: '6px' }}>
              Validation <span style={{ color: '#9ca3af', fontWeight: '400' }}>(optional)</span>
            </label>
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <input
                ...
                style={{ width: '110px', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' }}
              />
              <input
                type="number"
              ...
              <input
                ...
                style={{ flex: 1, minWidth: '160px', padding: '6px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' }}
              />
            </div>
          </div>
        </div>
...
export const FormFieldBuilder: React.FC<FormFieldBuilderProps> = ({ fields, onChange, projectId, hierarchyConfig }) => {
    ...
    if (hierarchyConfig?.levels && hierarchyConfig.levels.length > 0) {
      return [...hierarchyConfig.levels]
        ...
        .map(level => ({
          ...
          requiredMode: level.isRequired ? ('always' as const) : ('optional' as const),
          isFixed: true,
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: showPreview ? '1fr 360px' : '1fr', gap: '20px', alignItems: 'start' }}>
      {/* ── Builder Canvas ── */}
      <div>
        {/* Fixed-fields info */}
        <div style={{
          ...
          <strong>Student profile fields</strong> (Name, Email, Phone) are always pre-filled from the student's account.
          {hierarchyFixedFields.length > 0 ? (
            <> The <strong>{hierarchyFixedFields.map(f => f.fieldLabel || f.fieldName).join(' → ')}</strong> field{hierarchyFixedFields.length > 1 ? 's are' : ' is'} fixed by the project hierarchy and cannot be removed. Category options come from the <strong>Categories</strong> tab.</>
          ) : (
          ...
          <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', flexWrap: 'wrap' }}>
            {hierarchyFixedFields.map(f => (
              ...
              >
                ...
                {f.required && <span style={{ fontSize: '10px', color: '#ef4444', marginLeft: '2px' }}>*</span>}
                <span style={{
                  fontSize: '10px',
                  padding: '1px 5px',
                  borderRadius: '8px',
              ...
              {(provided: any) => (
                ...
                >
                  {fields.map((field, index) => {
                    ...
                    const condCount = (field.conditions || []).length;
                    const label = field.fieldLabel || field.fieldName || `Field ${index + 1}`;

                    // Trigger field names for this specific field (exclude itself)
                    const fieldTriggers = allFieldNames.filter(n => n !== field.fieldName);
                          ...
                          >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 14px' }}>
                              <span
                                ...
                                style={{ cursor: 'grab', color: '#9ca3af', display: 'flex', flexShrink: 0 }}
                              ...
                              <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', overflow: 'hidden', minWidth: 0 }}>
                                <span style={{
                                  fontSize: '14px',
                                  fontWeight: '500',
                                  color: field.fieldName ? '#1f2937' : '#9ca3af',
                                ...
                                {field.requiredMode === 'always' && (
                                  <span style={{ ...typeBadge(field.fieldType), background: '#fef2f2', color: '#ef4444' }}>required</span>
                                )}
                                {condCount > 0 && (
                                  <span
                                    style={condBadge}
                                    onClick={() => { setExpandedId(isExpanded ? null : (field.id || null)); }}
                              ...
                              <button
                                type="button"
                                onClick={() => setExpandedId(isExpanded ? null : (field.id || null))}
                                style={{ border: 'none', background: 'none', cursor: 'pointer', color: isExpanded ? '#3b82f6' : '#6b7280', display: 'flex', flexShrink: 0, padding: '2px' }}
                              ...
                              <button
                                ...
                                style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#ef4444', display: 'flex', flexShrink: 0, padding: '2px' }}
                                title="Remove field"
                              >
                                <MdDelete size={18} />
                              </button>
      ...
      {showPreview && (
        <div>
          <div style={{ position: 'sticky', top: '24px', background: 'white', border: '1px solid #e5e7eb', borderRadius: '12px', overflow: 'hidden' }}>
            <div style={{
              padding: '11px 16px',
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  style={{ fontSize: '12px', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}
                  title="Hide preview"
                >
            ...
            <div style={{ padding: '16px', maxHeight: '72vh', overflowY: 'auto' }}>
              {/* Fixed fields stub */}
              <div style={{ marginBottom: '16px' }}>
                ...
                {['Name *', 'Email *', 'Phone *'].map(n => (
                  <div
                    key={n}
                    style={{ padding: '8px 12px', background: '#f1f5f9', borderRadius: '6px', fontSize: '13px', color: '#64748b', marginBottom: '6px' }}
                  >
                    {n}
                  </div>
                ))}
              ...
              {projectId && hierarchyConfig && hierarchyConfig.levelCount >= 1 && (
                <div style={{ marginBottom: '14px' }}>
                  <HierarchyCategorySelector
                    ...
                    onChange={(val) => {
                      ...
                      // Push every selected level into previewData so condition rules react
                      const updates: Record<string, string> = {};
                      hierarchyConfig.levels.forEach(l => {
                        const key = `level${l.levelNumber}` as keyof CategoryHierarchyValue;
                        updates[l.displayName] = (val[key] as string) || '';
                      });
              ...
              <FormRenderer
                ...
                onChange={(name, val) => setPreviewData(prev => ({ ...prev, [name]: val }))}
                previewMode={false}
              />

              {fields.length === 0 && (
                <div style={{ fontSize: '12px', color: '#9ca3af', textAlign: 'center', padding: '8px' }}>
                  Add custom fields above to see them here. Interact to test conditional visibility.
                </div>
              )}
            </div>
          </div>
