    ...
    const fetchProjectConfiguration = async () => {
      try {
        ...
        if (hostname === "localhost" || hostname === "helpdesk.hubblehox.ai") {
          console.log(
            "Skipping project configuration fetch for super admin/main domain",
          );
          return;
        }

        const response = await fetch(
          `${API_CONFIG.BASE_URL}/api/projects/by-domain/${hostname}`,
        );
        if (response.ok) {
  ...
  const handleLogin = async (data: LoginFormData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const isSuperAdmin =
          roleCode === "SUPER_ADMIN" || roleCode === "Super Admin";
        if (isSuperAdmin) {
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                fontWeight: 600,
                cursor:
                  twoFactorOtp.length === 6 && !isLoading
                    ? "pointer"
                    : "not-allowed",
                opacity: isLoading ? 0.6 : 1,
                boxShadow:
            ...
            >
              <div>
                ...
                {otpForm.formState.errors.otp && (
                  <p
                    className="mt-2 text-body-small text-error-600"
                    role="alert"
                  >
          ...
          {announcementBanner.type === "rich" ? (
            <div
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(announcementBanner.message),
              }}
