...
import { MdExpandMore, MdExpandLess, MdSave, MdRefresh, MdCheckCircle, MdClose, MdContentCopy } from 'react-icons/md';
...
const CenterAssetMappingAccordion: React.FC = () => {
  ...
  const [centerSelections, setCenterSelections] = useState<{ [centerId: string]: CenterAssetSelection }>({});
  ...
  const canMapToCenter = hasPermission(PERMISSIONS.ASSET_MAP_TO_CENTER) || hasPermission(PERMISSIONS.ASSET_MANAGE);
  ...
  const fetchCenters = async () => {
    ...
    try {
      const token = localStorage.getItem('authToken');
      const response = await fetch(`${API_CONFIG.API_URL}/centers?projectId=${selectedProject}`, {
        headers: { 'Authorization': `Bearer ${token}` },
        credentials: 'include',
      });
      const data = await response.json();
  ...
  const toggleAssetForCenter = (centerId: string, assetId: string) => {
    setCenterSelections(prev => {
      ...
      const centerSelection = { 
        ...
        selectedAssets: existingSelection?.selectedAssets ? [...existingSelection.selectedAssets] : [],
        assetQuantities: existingSelection?.assetQuantities ? {...existingSelection.assetQuantities} : {},
      };
      ...
      if (isSelected) {
        ...
        centerSelection.selectedAssets = centerSelection.selectedAssets.filter(id => id !== assetId);
        delete centerSelection.assetQuantities[assetId];
      } else {
        // Add to selection
        centerSelection.selectedAssets = [...centerSelection.selectedAssets, assetId];
  ...
  const updateAssetQuantity = (centerId: string, assetId: string, quantity: number) => {
    setCenterSelections(prev => {
      const existingSelection = prev[centerId];
      const centerData = {
        centerId,
  ...
  const applyToAllCenters = (sourceAssets: string[], sourceQuantities: { [assetId: string]: number }) => {
    if (!window.confirm(`This will apply the same asset selection and quantities to all ${centers.length} centers. Continue?`)) {
      return;
    }
    
    // Find the source center to get audit date info
    const sourceCenterSelection = Object.values(centerSelections).find(
      sel => JSON.stringify(sel.selectedAssets) === JSON.stringify(sourceAssets)
    );
    ...
    showMessage('success', `Applied selection to all ${centers.length} centers`);
  };
  ...
  const toggleSelectAllAssetsForCenter = (centerId: string) => {
    ...
    const allSelected = selection?.selectedAssets?.length === filteredAssets.length;
  ...
  const saveMapping = async (centerId: string) => {
    const selection = centerSelections[centerId];
    if (!selection || selection.selectedAssets.length === 0) {
    ...
    for (const assetId of selection.selectedAssets) {
      ...
      if (totalAssigned > asset.predefinedCount) {
        overAllocatedAssets.push(`${asset.name} (${totalAssigned}/${asset.predefinedCount})`);
      }
    }

    if (overAllocatedAssets.length > 0) {
      showMessage('error', `Cannot save! Over-allocated assets: ${overAllocatedAssets.join(', ')}. Please reduce quantities.`);
      return;
    }
    ...
    try {
      ...
      if (errorCount === 0) {
        showMessage('success', `Mapping saved for ${selection.centerName} (${successCount} assets)`);
      } else {
        showMessage('error', `Saved ${successCount} assets, ${errorCount} failed`);
      }
  ...
  const saveAllMappings = async () => {
    ...
    try {
      ...
      if (errorCount === 0) {
        showMessage('success', `All mappings saved successfully (${successCount}/${totalAssets} assets)`);
      } else {
        showMessage('error', `Saved ${successCount}/${totalAssets} assets, ${errorCount} failed`);
      }
  ...
  const getTotalAssignedForAsset = useCallback((assetId: string): number => {
    return Object.values(centerSelections).reduce((total, selection) => {
      return total + (selection.assetQuantities[assetId] || 0);
    }, 0);
  }, [centerSelections]);

  const isAssetOverAllocated = useCallback((assetId: string): boolean => {
    const asset = assets.find(a => a._id === assetId);
  ...
  const filteredAssets = useMemo(() => {
    ...
    return assets.filter(asset => {
      ...
      const categoryName = asset.category 
        ? (typeof asset.category === 'object' ? (asset.category as any).name : asset.category).toLowerCase()
        : '';
      return assetName.includes(searchLower) || categoryName.includes(searchLower);
    });
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Center Name</th>
                      <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Mapped Assets</th>
                      <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Units Assigned</th>
                      <th className="px-6 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Validation</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {centers.length === 0 ? (
                      ...
                      centers.map((center) => {
                        ...
                        const totalUnits = centerMappings.reduce((sum, m) => sum + m.totalAssigned, 0);
                          ...
                          <tr key={center._id} className="hover:bg-gray-50">
                            ...
                            <td className="px-6 py-4">
                              {centerMappings.length === 0 ? (
                                <span className="text-sm text-gray-500">No assets mapped</span>
                              ) : (
                                <div className="space-y-1">
                                  {centerMappings.map((mapping) => (
                                    <div key={mapping._id} className="text-sm text-gray-900">
                                      • {mapping.assetId.name}
                                      {mapping.assetId.category && (
                                        <span className="ml-2 text-xs text-gray-500">
                                          ({typeof mapping.assetId.category === 'object' 
                                            ? (mapping.assetId.category as any).name 
                                            : mapping.assetId.category})
                                        </span>
                                      )}
                                    </div>
                            ...
                            <td className="whitespace-nowrap px-6 py-4 text-right text-sm text-gray-900">
                              {centerMappings.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                  {centerMappings.map((mapping) => (
                                    <div key={mapping._id}>
                                      {mapping.totalAssigned} {mapping.assetId.unit || 'units'}
                                    </div>
                                  ))}
                                </div>
                              )}
                            ...
                            <td className="whitespace-nowrap px-6 py-4 text-right">
                              {centerMappings.length === 0 ? (
                                ...
                                <div className="space-y-1">
                                  {centerMappings.map((mapping) => {
                                    const totalAssigned = getTotalAssignedForAsset(mapping.assetId._id);
                                    const predefinedCount = mapping.assetId.predefinedCount || 0;
                                    const isValid = totalAssigned <= predefinedCount;
                                      ...
                                      <div key={mapping._id}>
                                        {isValid ? (
                                          <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-2 py-1 text-xs font-semibold text-green-800">
                                            ...
                                            Valid ({totalAssigned}/{predefinedCount})
                                          </span>
                                        ) : (
                                          <span className="inline-flex rounded-full bg-red-100 px-2 py-1 text-xs font-semibold text-red-800">
                                            Over ({totalAssigned}/{predefinedCount})
                                          </span>
                                        )}
                                      </div>
                                    );
            ...
            <div className="flex items-start gap-3">
              <div className="flex-shrink-0">
                {message.type === 'success' ? (
                  ...
                  <div className="flex h-6 w-6 items-center justify-center rounded-full bg-red-600 text-white text-xl font-bold">!</div>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold mb-1">
                  {message.type === 'success' ? 'Success' : 'Error - Action Required'}
                </p>
      ...
      <div className="p-6">
        ...
        {selectedProject && centers.length > 0 && (
          <>
            ...
            {assets.length > 0 && (
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-3">Asset Allocation Overview - All Centers</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {assets.slice(0, 6).map((asset) => {
                  const totalAssigned = getTotalAssignedForAsset(asset._id);
                  const available = Math.max(0, asset.predefinedCount - totalAssigned);
                  const percentageUsed = asset.predefinedCount > 0 
                    ? Math.round((totalAssigned / asset.predefinedCount) * 100) 
                    : 0;
                  const isOverAllocated = totalAssigned > asset.predefinedCount;
                  const isNearLimit = percentageUsed >= 80 && !isOverAllocated;
                    ...
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          {asset.icon && <span className="text-xl flex-shrink-0">{asset.icon}</span>}
                          <div className="flex-1 min-w-0">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex justify-between text-xs">
                          ...
                          <span className={`font-semibold ${isOverAllocated ? 'text-red-700' : 'text-blue-700'}`}>
                ...
                {assets.length > 6 && (
                  <div className="rounded-lg border border-gray-200 bg-white p-3 flex items-center justify-center">
                    <div className="text-center">
                      <p className="text-sm font-medium text-gray-700">+{assets.length - 6} more</p>
                      <p className="text-xs text-gray-500 mt-1">View in asset selection below</p>
                    </div>
                  </div>
                )}
              </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-600">
                      Configure assets for one center, then apply to all centers at once
                    </p>
                  </div>
                  <button
            ...
            <div className="space-y-3">
              {centers.map((center) => {
                ...
                return (
                  <div key={center._id} className="overflow-hidden rounded-lg bg-white shadow">
                    {/* Accordion Header */}
                    <div
                      ...
                      <div className="flex items-center gap-2">
                        {canMapToCenter && selectedCount > 0 && (
                          <div className="flex items-center gap-2 mr-2 bg-purple-50 border border-purple-200 rounded-lg px-3 py-1.5">
                            <span className="text-xs font-medium text-purple-900">📅 Audit Schedule:</span>
                            <input
                              type="date"
                              ...
                              value={
                                selection?.auditStartDate 
                                  ? new Date(selection.auditStartDate).toISOString().split('T')[0]
                            ...
                            {selection?.nextAuditDate && (
                              <>
                                ...
                                <span className="text-xs text-purple-900 font-semibold">
                                  {new Date(selection.nextAuditDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                                </span>
                              </>
                            )}
                          </div>
                        ...
                        {canMapToCenter && selectedCount > 0 && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              applyToAllCenters(selection.selectedAssets, selection.assetQuantities);
                            }}
                    ...
                    {isExpanded && (
                      <div className="border-t border-gray-200 bg-gray-50 px-6 py-4">
                        <p className="text-sm text-gray-600 mb-4">Configure asset allocation for this center. Changes will update the overview dashboard above.</p>
                        ...
                        <div className="mb-4">
                          <div className="flex items-center justify-between mb-3">
                            <label className="text-sm font-medium text-gray-700">
                              Select Assets & Set Quantities for {center.centerName}
                            </label>
                            <button
                              onClick={() => toggleSelectAllAssetsForCenter(center._id)}
                              disabled={!canMapToCenter}
                              className="text-xs font-medium text-blue-600 hover:text-blue-800 disabled:opacity-50"
                            >
                              {selection?.selectedAssets?.length === filteredAssets.length ? 'Deselect All' : 'Select All'} ({filteredAssets.length})
                            </button>
                          </div>
                          ...
                          <div className="rounded-lg border border-gray-300 bg-white max-h-96 overflow-y-auto">
                            ...
                            <div className="divide-y divide-gray-200">
                              {filteredAssets.map((asset) => {
                                const isSelected = selection?.selectedAssets?.includes(asset._id) || false;
                                const isOverAllocated = isAssetOverAllocated(asset._id);
                                const totalAssigned = getTotalAssignedForAsset(asset._id);
                                const currentQuantity = selection?.assetQuantities?.[asset._id] || 0;
                                const categoryName = asset.category 
                                  ? (typeof asset.category === 'object' ? (asset.category as any).name : asset.category)
                                  : '';
                                  ...
                                  >
                                    <div 
                                      ...
                                      onClick={(e) => {
                                        ...
                                        e.preventDefault();
                                        e.stopPropagation();
                                        toggleAssetForCenter(center._id, asset._id);
                                      }}
                                    >
                                      <input
                                        type="checkbox"
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center gap-2">
                                        {asset.icon && <span className="text-lg flex-shrink-0">{asset.icon}</span>}
                                      ...
                                      <div className="mt-0.5 flex items-center gap-2 text-xs text-gray-600">
                                        <span>Available: {asset.predefinedCount} {asset.unit || 'units'}</span>
                                        {isOverAllocated && isSelected && (
                                          <span className="font-semibold text-red-700 flex items-center gap-1">
                                            <span>⚠️</span>
                                            Over-allocated: {totalAssigned}/{asset.predefinedCount}
                                          </span>
                                        )}
                                      </div>
                                    </div>
                                      ...
                                      <div className="flex items-center gap-2 flex-shrink-0">
                                        <div className="text-right">
                                          ...
                                          <input
                                            ...
                                            onChange={(e) => {
                                              e.stopPropagation();
                                              updateAssetQuantity(center._id, asset._id, parseInt(e.target.value) || 0);
                                            }}
                                            onClick={(e) => e.stopPropagation()}
                                            disabled={!canMapToCenter}
                          ...
                          <p className="mt-2 text-xs text-gray-500">
                            {selection?.selectedAssets?.length || 0} of {assets.length} assets selected
                          </p>
                        </div>

                        {/* Selected Assets Summary */}
                        {selection?.selectedAssets && selection.selectedAssets.length > 0 && (
                          <div className="mb-4 rounded-lg border border-blue-200 bg-blue-50 p-3">
                            <div className="flex items-start justify-between gap-3">
                              <div className="flex-1">
                                <h5 className="text-sm font-semibold text-gray-900 mb-2">Selected Assets Summary</h5>
                                <div className="flex flex-wrap gap-2">
                                  {selection.selectedAssets.map((assetId) => {
                                    const asset = assets.find(a => a._id === assetId);
                                    if (!asset) return null;
                                    const quantity = selection.assetQuantities[assetId] || 0;
                                    
                                    return (
                                      <span key={assetId} className="inline-flex items-center gap-1.5 rounded-full bg-white border border-blue-200 px-3 py-1 text-xs">
                                        {asset.icon && <span>{asset.icon}</span>}
                                        <span className="font-medium">{asset.name}</span>
                                        <span className="text-blue-600 font-semibold">×{quantity}</span>
                                        <button
                                          onClick={() => toggleAssetForCenter(center._id, assetId)}
                        ...
                        {assets.length === 0 && (
                          <div className="rounded-lg bg-white p-8 text-center border border-gray-200">
                            <p className="text-gray-600">No assets found for this project. Please add assets first.</p>
                          </div>
                        )}
                      </div>
                    )}
        ...
        {selectedProject && centers.length === 0 && !loading && (
          <div className="rounded-lg bg-white p-8 text-center shadow">
            <p className="text-gray-600">No centers found for this project. Please add centers first.</p>
          </div>
        )}

        {!selectedProject && (
          <div className="rounded-lg bg-white p-8 text-center shadow">
            <p className="text-gray-600">Please select a project to begin mapping assets to centers.</p>
          </div>
        )}
      </div>
    </DashboardLayout>
