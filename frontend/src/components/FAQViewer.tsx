...
const FAQViewer: React.FC = () => {
  ...
  const { branding: fullBranding } = useBranding();

  // Resolve projectId: prefer BrandingContext (no extra API call), fall back to localStorage
  const projectId = useMemo(() => {
    if (fullBranding?.projectId) return fullBranding.projectId;
    try {
      const projectContext = localStorage.getItem("projectContext");
      if (projectContext) return JSON.parse(projectContext).projectId ?? null;
    } catch {}
    return null;
  }, [fullBranding]);
            ...
            return (
              ...
              >
                ...
                >
                  {/* Question Number */}
                  <div
                    className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl flex items-center justify-center text-sm sm:text-base font-bold transition-all ${
                      isExpanded
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {faq.category && (
                      <span
                        className={`inline-flex items-center mt-2 px-2.5 py-1 rounded-full text-xs font-medium transition-colors ${
                          isExpanded
                  ...
                  <div
                    className={`flex-shrink-0 w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all ${
                      isExpanded
