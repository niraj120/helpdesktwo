...
const HierarchyConfigManager: React.FC<HierarchyConfigManagerProps> = ({
  ...
  const fetchCategoryTree = useCallback(async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("authToken");
      const response = await axios.get(
        `${API_URL}/hierarchy-config/${projectId}/tree`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchPriorities = useCallback(async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/sla-rules?projectId=${projectId}&isActive=true`,
        {
          headers: {
  ...
  const handleLevelCountChange = (count: number) => {
    ...
    for (let i = 1; i <= count; i++) {
      ...
      levels.push({
        levelNumber: i,
        displayName:
          existingLevel?.displayName || defaultNames[i - 1] || `Level ${i}`,
        isMandatory: i === 1 ? true : existingLevel?.isMandatory || false,
        isActive: true,
      });
    }

    // Rebuild visibilitySettings so all new levels are visible in all views
    const allLevels = Array.from({ length: count }, (_, i) => i + 1);
    const existingVis = (editedConfig as any)?.visibilitySettings;
    const visibilitySettings = {
      showInOnlineForm: allLevels,
      showInOfflineForm: allLevels,
      showInTicketDisplay: allLevels,
      showInFilters: existingVis?.showInFilters ?? [1, 2],
    };
   ...
   */
  const handleLevelConfigChange = (
    levelNumber: number,
    field: keyof HierarchyLevel,
    value: any,
  ) => {
  ...
  const handleSubmitCategory = async () => {
    ...
    try {
      ...
      if (modalState.mode === "add") {
        ...
        const newLevel = modalState.parentLevel
          ? modalState.parentLevel + 1
          : 1;
        const autoCode = getNextCode(); // Gets max code across entire project + 1

        await axios.post(
          `${API_URL}/hierarchy-config/${projectId}/categories`,
          {
  ...
  const parseCSV = (csvText: string) => {
    ...
    let startIndex = 0;
    if (
      lines[0]?.toLowerCase().includes("level") &&
      ...
      <div className="mb-6 p-4 bg-white border border-gray-200 rounded-lg">
        ...
        <div className="mb-4">
          ...
          <p className="text-xs text-gray-500 mb-2">
            Select which category level determines the ticket priority
            automatically
          </p>
          <div className="flex gap-2 flex-wrap">
            <button
              onClick={() =>
                setEditedConfig({ ...editedConfig, priorityFromLevel: 0 })
              }
              className={`
            ...
            {editedConfig?.levels?.map((level) => (
              <button
                ...
                onClick={() =>
                  setEditedConfig({
                    ...editedConfig,
                    priorityFromLevel: level.levelNumber,
                  })
                }
                className={`
                  px-3 py-1.5 rounded-lg border text-sm transition-colors
          ...
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm mx-4">
            <div className="px-6 py-4">
              ...
              <p className="text-gray-600">
                ...
                <strong>{deleteConfirm.category?.name}</strong>"? This action
                cannot be undone.
              </p>
            </div>
            <div className="px-6 py-4 border-t border-gray-200 flex justify-end gap-2">
            ...
            <div className="px-6 py-4 flex-1 overflow-y-auto">
              ...
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm text-blue-800">
                ...
                <ul className="list-disc ml-4 space-y-1">
                  <li>
                    <strong>Level:</strong> 1-4 (based on your hierarchy)
                  </li>
                  <li>
                    <strong>ParentName:</strong> Leave empty for Level 1, exact
                    parent name for others
                  </li>
                  <li>
                    <strong>Name:</strong> Category name (required)
                  </li>
                  <li>
                    <strong>DefaultPriority:</strong> Optional (high, medium,
                    normal, low)
                  </li>
                </ul>
              </div>
                    ...
                    <table className="w-full text-sm">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {bulkUpload.parsedData.slice(0, 50).map((row, idx) => (
                          <tr
                            key={idx}
                            className={`hover:bg-gray-50 ${row.code ? "bg-yellow-50" : ""}`}
                          >
