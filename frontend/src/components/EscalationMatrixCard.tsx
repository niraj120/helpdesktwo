...
const EscalationMatrixCard: React.FC<EscalationMatrixCardProps> = ({
  ...
  const selectedLevel = allowedLevels.find(
    (l) => l.levelId === selectedLevelId,
  );
  const hasMultipleUsers =
    selectedLevel?.users && selectedLevel.users.length > 1;
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Escalate To ({allowedLevels.length} level
              {allowedLevels.length !== 1 ? "s" : ""} available)
            </label>
            {allowedLevels.length === 0 ? (
              <div className="p-3 bg-gray-50 rounded-lg">
                <p className="text-sm text-gray-500 flex items-center">
                  <InformationCircleIcon className="w-5 h-5 mr-2" />
                  No escalation levels available. This ticket may be at the
                  highest level or no matrix is assigned.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        const selectedLvl = allowedLevels.find(
                          (l) => l.levelId === e.target.value,
                        );
                    ...
                    <div className="ml-3 flex-grow">
                      ...
                      <p className="text-xs text-gray-500">
                        {level.previousHandlerName ? (
                          ...
                          <span className="mr-2">
                            {level.previousHandlerName} - {level.roleName}
                          </span>
                        ) : level.userNames && level.userNames.length > 0 ? (
                          <span className="mr-2">
                            {level.userNames.join(", ")} - {level.roleName}
                          </span>
                        ) : (
                          level.roleName && (
                            <span className="mr-2">Role: {level.roleName}</span>
                          )
                        )}
                        <span>
                          SLA: {level.slaHours}
          ...
          <div className="flex space-x-2">
            {allowedLevels.length > 0 && (
              <button
                ...
                disabled={
                  !selectedLevelId || !escalationReason.trim() || submitting
                }
