...
export const HeaderProjectSwitcher: React.FC = () => {
  ...
  console.log(
    "🎨 HeaderProjectSwitcher: userProjects from context:",
    userProjects,
  );
  console.log(
    "🎨 HeaderProjectSwitcher: userProjects.length:",
    userProjects.length,
  );
  ...
  const favoriteProjectsList = filteredProjects.filter((p) =>
    favoriteProjects.includes(p._id),
  );
  const recentProjectsList = filteredProjects.filter(
    (p) => recentProjects.includes(p._id) && !favoriteProjects.includes(p._id),
  );
  const otherProjectsList = filteredProjects.filter(
    (p) => !favoriteProjects.includes(p._id) && !recentProjects.includes(p._id),
  );
  ...
  const userRoleFromStorage =
    ...
    "";
  if (
    userRoleFromStorage === "Super Admin" ||
    userRoleFromStorage === "SUPER_ADMIN"
  ) {
    console.log(
      "🚫 HeaderProjectSwitcher: Super Admin detected - hiding project switcher",
    );
    return null;
  }
  ...
  if (userProjects.length === 0) {
    console.log(
      "🚫 HeaderProjectSwitcher: No projects assigned - hiding project switcher",
    );
    return null;
  }
        ...
        <div style={{ textAlign: "left" }}>
          ...
          >
            {viewMode === "unified"
              ...
              : currentProject?.name ||
                getText("Select Project", "प्रकल्प चुनें", "प्रकल्प निवडा")}
          </div>
              ...
              <>
                <div
                  ...
                  style={{
                    ...
                    marginBottom: "12px",
                    backgroundColor:
                      viewMode === "unified" ? "#f3f4f6" : "transparent",
                    transition: "background-color 0.2s",
                  }}
                  onMouseEnter={(e) => {
                    if (viewMode !== "unified")
