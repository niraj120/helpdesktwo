...
const AddProjectForm = ({ project, onClose, onSave }: AddProjectFormProps) => {
  ...
  const [formData, setFormData] = useState({
    ...
    // Note: Offline centers are now managed in Offline Module Settings
    // Note: Online form fields can be customized in Query Configuration

    // File Download Settings
    fileDownloadPermission: "logged-in-permission" as
  ...
  const [users, setUsers] = useState<
    Array<{
      ...
      role:
        | string
        | { _id: string; code: string; name: string; permissions?: string[] };
      roleId?: string;
      projects?: string[];
    }>
  >([]);
  ...
  const rolesWithAssignPermission = projectRoles.filter((role) =>
    hasAssignTicketPermission(role),
  );

  // DEBUG: Log roles data
  useEffect(() => {
    console.log("ðŸ” DEBUG - Full Project Object:", project);
    console.log("ðŸ” DEBUG - Roles State:", roles.length, roles);
    console.log("ðŸ” DEBUG - Project ID:", project?._id);
    console.log("ðŸ” DEBUG - Project id (lowercase):", project?.id);
    console.log("ðŸ” DEBUG - Project ID Type:", typeof project?._id);
    const actualProjectId = project?._id || project?.id;
    console.log("ðŸ” DEBUG - Actual Project ID to use:", actualProjectId);
    roles.forEach((role) => {
      console.log(
        `   Role: ${role.name}, Type: ${role.type}, Projects:`,
        role.projects,
        "ProjectId:",
        role.projectId,
      );
      if (role.projects && Array.isArray(role.projects)) {
        ...
        role.projects.forEach((p, i) => {
          console.log(
            ...
            String(p),
            "| Match:",
            String(p) === String(actualProjectId),
          );
        });
      }
      if (role.projectId) {
        console.log(
          `      ProjectId match:`,
          String(role.projectId) === String(actualProjectId),
        );
      }
    });
    console.log(
      "ðŸ” DEBUG - Filtered ProjectRoles:",
      projectRoles.length,
      projectRoles,
    );
    console.log(
      "ðŸ” DEBUG - ProjectRoles names:",
      projectRoles.map((r) => r.name),
    );
  ...
  useEffect(() => {
    ...
    if (hasFetchedMasterData.current) {
      console.log("â­ï¸ Skipping duplicate master data fetch (already loaded)");
      return;
    }

    const fetchMasterData = async () => {
      try {
        console.log("ðŸ”„ Fetching master data: countries, users, roles...");
        ...
        const [countriesRes, usersRes, rolesRes] = await Promise.all([
          fetch(`${API_BASE_URL}/master/countries`, {
            headers,
            credentials: "include",
          }),
  ...
  useEffect(() => {
    if (project) {
      ...
      setFormData((prevData) => ({
        ...
        privacyPolicyUrl:
          project?.configuration?.footerLinks?.privacyPolicy || "",
        cookiePolicyUrl:
          project?.configuration?.footerLinks?.cookiePolicy || "",

        // Announcement Banner
        announcementBannerMessage:
          project?.configuration?.announcementBanner?.message || "",
        announcementBannerType:
          project?.configuration?.announcementBanner?.type || "plain",

        // Ticket Assignment Settings
        enableAutoAssignment:
          project?.configuration?.ticketAssignmentSettings?.enabled ?? false,
        assignmentType:
          project?.configuration?.ticketAssignmentSettings?.assignmentType ||
          "manual",
        assignToUsers:
          project?.configuration?.ticketAssignmentSettings?.assignToUsers || [],
        assignToRoles:
          project?.configuration?.ticketAssignmentSettings?.assignToRoles || [],
        ...
        notifyOnAssignment:
          project?.configuration?.ticketAssignmentSettings
            ?.notifyOnAssignment ?? true,
        conditionRules:
          project?.configuration?.ticketAssignmentSettings?.conditionRules ||
          [],
        ...
        fileDownloadPermission:
          project?.configuration?.fileSettings?.downloadPermission ||
          "logged-in-permission",
        inlineImageAttachment:
          project?.configuration?.fileSettings?.inlineImageAttachment ?? false,
        attachFilesToEmails:
          project?.configuration?.fileSettings?.attachToEmails ?? false,

        // Login Settings
        enableFormLogin:
          project?.configuration?.loginSettings?.enableFormLogin ?? true,
        enableGoogleRecaptcha:
          project?.configuration?.loginSettings?.enableGoogleRecaptcha ?? false,
        keycloakSsoEnabled:
          project?.configuration?.loginSettings?.ssoSettings?.keycloak
            ?.enabled ?? false,
        keycloakUrl:
          project?.configuration?.loginSettings?.ssoSettings?.keycloak?.url ||
          "",
        keycloakRealm:
          project?.configuration?.loginSettings?.ssoSettings?.keycloak?.realm ||
          "",
        keycloakClientId:
          project?.configuration?.loginSettings?.ssoSettings?.keycloak
            ?.clientId || "",
        ...
        // Security Settings
        allowUserSignup:
          project?.configuration?.securitySettings?.allowUserSignup ?? true,
        restrictSignupViaSocial:
          project?.configuration?.securitySettings?.restrictSignupViaSocial ??
          false,
        enforceTwoFactorAuth:
          project?.configuration?.securitySettings?.mfaRequired ?? false,
        passwordPolicy: project?.configuration?.securitySettings?.passwordPolicy
          ? "custom"
          : "default",
        requireLowerCase:
          project?.configuration?.securitySettings?.passwordPolicy
            ?.requireLowercase ?? false,
        requireUpperCase:
          project?.configuration?.securitySettings?.passwordPolicy
            ?.requireUppercase ?? false,
        ...
        minimumPasswordLength:
          project?.configuration?.securitySettings?.passwordPolicy?.minLength ||
          8,
        passwordExpiration: (() => {
          const days =
            project?.configuration?.securitySettings?.passwordPolicy
          ...
          if (days === 30 || days === 60 || days === 90 || days === 180)
            return String(days) as "30" | "60" | "90" | "180";
          return "never";
        })(),

        // Knowledge Base Settings
        enableKB:
          project?.configuration?.knowledgeBaseSettings?.enabled ?? true,
        kbHomeConfiguration:
          project?.configuration?.knowledgeBaseSettings?.kbHomeConfiguration ||
          prevData.kbHomeConfiguration,
        articleConfiguration:
          project?.configuration?.knowledgeBaseSettings?.articleConfiguration ||
          prevData.articleConfiguration,
        enableAIAssistance:
          project?.configuration?.knowledgeBaseSettings?.enableAIAssistance ??
          false,
        enableSatisfactionFeedback:
          project?.configuration?.knowledgeBaseSettings
            ?.enableSatisfactionFeedback ?? true,
        satisfactionFeedback:
          project?.configuration?.knowledgeBaseSettings?.satisfactionFeedback ||
          prevData.satisfactionFeedback,
        seoSettings:
          project?.configuration?.knowledgeBaseSettings?.seoSettings ||
          prevData.seoSettings,
        ...
        themeColor:
          ...
          "#444ce7",
        customCSS:
          project?.configuration?.customizationSettings?.customCSS || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const generatedCode = formData.portalName
        .replace(/\s+/g, "")
        .toUpperCase()
        .substring(0, 10);
          ...
          loginSettings: {
            ...
            ssoSettings: {
              keycloak: {
                ...
                ...(formData.keycloakClientSecret
                  ? { clientSecret: formData.keycloakClientSecret }
                  : {}),
              },
            },
          },
            ...
            passwordPolicy:
              formData.passwordPolicy === "custom"
                ? {
                    ...
                    expiryDays:
                      formData.passwordExpiration === "never"
                        ? 0
                        : parseInt(formData.passwordExpiration),
                  }
                : {
                    minLength: 8,
  ...
  const tabs = [
    ...
    },
    {
      id: "ticketportal",
      label: "Ticket Portal",
      labelMr: "à¤Ÿà¤¿à¤•à¤Ÿ à¤ªà¥‹à¤°à¥à¤Ÿà¤²",
      icon: <MdConfirmationNumber />,
    },
    {
      id: "customization",
      label: "Customization",
      labelMr: "à¤¸à¤¾à¤¨à¥à¤•à¥‚à¤²à¥€à¤•à¤°à¤£",
      icon: <MdPalette />,
    },
  ];
              ...
              {activeTab === "general" && (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span style={{ color: "#E6393E", fontWeight: "600" }}>
                        *
                      </span>{" "}
                  ...
                  <div>
                    ...
                    >
                      The portal name will be displayed in the customer portal
                      header.
                    </p>
                    <input
                      type="text"
                      ...
                      style={{
                        ...
                        backgroundColor: errors.portalName
                          ? "rgba(239, 68, 68, 0.02)"
                          : undefined,
                      }}
                    />
                  ...
                  <div>
                    ...
                    >
                      This will be displayed in the browser tab when customers
                      access the portal.
                    </p>
                    <input
                      type="text"
                      ...
                      onBlur={() => {
                        if (!formData.browserTabTitle.trim()) {
                          setErrors({
                            ...errors,
                            browserTabTitle: "Browser tab title is required",
                          });
                        }
                      }}
                      ...
                      style={{
                        width: "100%",
                        boxSizing: "border-box",
                        borderColor: errors.browserTabTitle
                          ? "#ef4444"
                          : undefined,
                        backgroundColor: errors.browserTabTitle
                          ? "rgba(239, 68, 68, 0.02)"
                          : undefined,
                      }}
                    />
                  ...
                  <div>
                    ...
                    >
                      This text will appear at the bottom of the login page and
                      customer portal.
                    </p>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    >
                      Enter a custom URL path for this project. Users will
                    ...
                    >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            logoLinkbackUrl: e.target.value,
                          })
                        }
                  ...
                  <div>
                    ...
                    >
                      Footer links will be shown on all pages in the customer
                      portal. The footer links will be hidden when left empty.
                    </p>
                  ...
                  <div>
                    ...
                    >
                      The announcement banner can be used to communicate an
                      important message to the users. It appears on the top of
                      the login page for all project-specific logins (Admin,
                      Agent, Counselor, Center Manager) except Student Login.
                    </p>
                    ...
                    >
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              keycloakSsoEnabled: e.target.checked,
                            })
                          }
                          style={{
                            width: "18px",
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            Allow users to sign in via Keycloak OpenID Connect
                            (PKCE). An "Sign in with SSO" button will appear on
                      ...
                      {formData.keycloakSsoEnabled && (
                        ...
                        >
                          ...
                          >
                            Configure the Keycloak realm and client details
                            below. Leave fields blank to use the server-wide
                            defaults from <code>.env</code>.
                          </p>
                          ...
                          >
                            The SSO callback URL to register in Keycloak:{" "}
                            <strong>
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              enableFormLogin: e.target.checked,
                            })
                          }
                          style={{
                            width: "18px",
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            Enabling this option will allow users to sign into
                            the customer portal using a username and password.
                            Turning off this option will hide the form login
                            option in the customer portal.
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              enableGoogleRecaptcha: e.target.checked,
                            })
                          }
                          style={{
                            width: "18px",
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          >
                            Enabling this option allows users to verify with
                            Google reCAPTCHA during sign up and sign in.
                          </p>
                        </div>
                      </div>
                    </div>
              ...
              {activeTab === "security" && (
                ...
                */}
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            enforceTwoFactorAuth: e.target.checked,
                          })
                        }
                        style={{
                          width: "18px",
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          This will require all users to verify their login with
                          both password and mobile number OTP. This applies only
                          to project-specific logins (Admin, Agent, Counselor,
                          Center Manager), not to student logins.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    >
                      The password policy is only applicable if you have allowed
                      form logins.
                    </p>
                          ...
                          >
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  minimumPasswordLength: parseInt(
                                    e.target.value,
                                  ),
                                })
                              }
                          ...
                          >
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  passwordExpiration: e.target.value as any,
                                })
                              }
                              className="text-field"
                              style={{
              ...
              {activeTab === "ticketportal" && (
                ...
                >
                      ...
                      >
                        <input
                          ...
                          onChange={(e) =>
                            setFormData({
                              ...formData,
                              ticketSubmissionMode: e.target.value as
                                | "online"
                                | "offline"
                                | "both",
                            })
                  ...
                  >
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          ticketSubmissionAnnouncement: e.target.value,
                        })
                      }
                  ...
                  >
                    ...
                    >
                      Configure how tickets should be assigned to agents in your
                      project.
                    </p>
                      ...
                      <div style={{ marginBottom: "20px" }}>
                        ...
                        >
                          ...
                          <option value="round-robin">
                            Round Robin (Auto-assign to agents)
                          </option>
                        </select>
                      ...
                      {formData.assignmentType === "round-robin" && (
                        <div style={{ marginBottom: "20px" }}>
                          ...
                          >
                            ...
                            <br />
                            <br />
                            Agents are identified by roles marked as "Agent
                            Role" in RBAC Setup. Only users with agent roles
                            mapped to this project will receive auto-assigned
                            tickets.
                          </div>
                        </div>
                      )}
                      ...
                      {formData.assignmentType === "manual" && (
                        <div style={{ marginBottom: "20px" }}>
                          {!project?._id ? (
                            ...
                            >
                              âš ï¸ No roles with "Assign Ticket" permission are
                              available. Create or edit a role in RBAC Setup to
                              add TICKET_ASSIGN permission.
                            </div>
                          ) : (
                            <>
                              <label
                              ...
                              >
                                ...
                                shown. Users with this role will have access to
                                the Assign Tickets page where they can assign
                                tickets to any agent in the project.
                                {rolesWithAssignPermission.some(
                                  (r) => !getMappedStatus(r),
                                ) && (
                                  <span
                                    style={{
                          ...
                          >
                            <input
                              ...
                              onChange={(e) =>
                                setFormData({
                                  ...formData,
                                  reassignOnEscalation: e.target.checked,
                                })
                              }
                              style={{
                                width: "16px",
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            ticketWelcomeMessage: e.target.value,
                          })
                        }
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setFormData({
                            ...formData,
                            ticketSuccessMessage: e.target.value,
                          })
                        }
                              ...
                              >
                                ...
                                >
                                  ...
                                  <select
                                    ...
                                    onChange={(e) => {
                                      ...
                                      newFields[index].fieldType = e.target
                                        .value as any;
                                      setFormData({
                                        ...formData,
                                        onlineFormFields: newFields,
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      setFormData({
                                        ...formData,
                                        onlineFormFields:
                                          formData.onlineFormFields.filter(
                                            (_, i) => i !== index,
                                          ),
                                      });
                                    }}
                                ...
                                {field.fieldType === "dropdown" && (
                                  <div style={{ marginTop: "12px" }}>
                                    ...
                                    >
                                      Dropdown Options (one per line or
                                      comma-separated)
                                    </label>
                                    <textarea
                ...
                >
                    ...
                    <div style={{ marginBottom: "20px" }}>
                      ...
                      >
                        ...
                        <button
                          ...
                          onClick={() =>
                            document
                              .getElementById("loginBgImageInput")
                              ?.click()
                          }
                          style={{
                            padding: "8px 16px",
                  ...
                  <div style={{ marginBottom: "32px" }}>
                    ...
                    <div
                      style={{
                        marginBottom: "20px",
                        opacity: formData.themeMode === "dark" ? 0.5 : 1,
                        pointerEvents:
                          formData.themeMode === "dark" ? "none" : "auto",
                      }}
                    >
                      <label
                        style={{
                      ...
                      >
                        Select a color for the overall theme of the customer
                        portal.
                      </div>
                  ...
                  <div style={{ marginBottom: "32px" }}>
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          Any custom code you add to the customer portal is your
                          responsibility. Please take care when making these
                          modifications because they could impact your customer
                          experience. When incorporating CSS or JS, please use
                          proper syntax.
                        </p>
                      </div>
                    </div>
              ...
              >
                ...
                >
                  {saving && (
                    ...
                    >
                      <path
                        d="M8 2v2M8 12v2M14 8h-2M4 8H2M12.657 12.657l-1.414-1.414M4.757 4.757L3.343 3.343M12.657 3.343l-1.414 1.414M4.757 11.243l-1.414 1.414"
                        stroke="currentColor"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
          ...
          >
            ...
            <div style={{ padding: "24px" }}>
              ...
              >
                ...
                >
                  Your custom domain path will be used to access the customer
                  portal (help center). To access the agent portal, add{" "}
                  <strong>/agent</strong> at the end of the URL.
                </p>
                <p
                  style={{
                    fontSize: "13px",
                ...
                >
                  ...
                  >
                    https://www.hubblehox.com/
                    {formData.portalUrl || "domain_name"}/agent
                  </code>
                </p>
              </div>
              ...
              >
                ...
                >
                  The domain path must be unique and will be mapped to your
                  organization's portal. Changes may take a few minutes to
              ...
              <button
                ...
                onClick={() => {
                  if (formData.portalUrl) {
                    ...
                    alert(
                      `Domain path "${formData.portalUrl}" has been mapped successfully!`,
                    );
                  }
                }}
        ...
        >
          ...
          >
            ...
            <div style={{ padding: "24px" }}>
              ...
              >
                This is how the announcement banner will appear on the login
                screen for Admin, Agent, Counselor, and Center Manager logins.
              </p>
          ...
          >
            ...
            >
              {toast.type === "success" && (
                <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                  <path
                    d="M13.3 4.7L6 12L2.7 8.7L3.4 8L6 10.6L12.6 4L13.3 4.7Z"
                    fill="#10b981"
                  />
                </svg>
              )}
