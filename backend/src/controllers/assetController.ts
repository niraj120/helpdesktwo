...
export const createAsset = async (req: Request, res: Response) => {
  try {
    const { projectId, name, description, category, predefinedCount, unit } =
      req.body;
...
export const updateAsset = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, description, category, predefinedCount, unit, isActive } =
      req.body;
    const user = (req as any).user;
    const userId = user.userId;
    const userName =
...
export const deleteAsset = async (req: Request, res: Response) => {
  try {
    ...
    const mappingCount = await CenterAssetMapping.countDocuments({
      assetId: id,
    });
