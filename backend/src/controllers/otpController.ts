...
export const sendPhoneOtp = async (req: Request, res: Response) => {
  try {
    ...
    // Send OTP via SMS
    const result = await sendOTPSMS(projectId, phone, otp);

    if (result.success) {
      console.log(`📱 OTP sent to phone ${phone.slice(-4).padStart(phone.length, '*')}`);
      return res.json({
        success: true,
...
export const sendEmailOtp = async (req: Request, res: Response) => {
  try {
    ...
    if (success) {
      console.log(`📧 OTP sent to email ${email.replace(/(.{2}).*@/, '$1***@')}`);
      return res.json({
        success: true,
        message: 'OTP sent successfully',
        otpKey
...
export const verifyOtp = async (req: Request, res: Response) => {
  try {
    ...
    if (isValid) {
      console.log(`✅ OTP verified for ${type}: ${value?.slice?.(-4)?.padStart?.(value?.length || 4, '*') || 'unknown'}`);
      return res.json({
        success: true,
