...
export const getMyAssets = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(
      "Looking for mappings with centerIds:",
      centerIds.map((id) => id.toString()),
    );
    console.log(
      "Or legacy mappings with projectIds:",
      projectIds.map((id) => id.toString()),
    );
    ...
    const centerMap = centers.reduce(
      (acc, center) => {
    ...
    for (const mapping of mappings) {
      ...
      if (mappingCenterId) {
        ...
        const centerName =
          typeof centerDoc === "object"
            ? centerDoc.centerName
            : centerMap[mappingCenterId.toString()];
        ...
        const matchingCenters = centers.filter(
          (c) => c.projectId?.toString() === String(mapping.projectId),
        );

        console.log(
          `Legacy mapping ${mapping._id} (project: ${mapping.projectId}) matches ${matchingCenters.length} user centers`,
        );
    ...
    // Deduplicate: the same assetId+centerId can appear from both the new per-center mapping
    // AND the legacy project-level mapping paths. Prefer the new per-center record (added
    // first in the loop since the $or puts centerId matches before project-level matches).
    const seen = new Set<string>();
    const dedupedMappings = formattedMappings.filter((m) => {
      // assetId: populated document after toObject() → plain object with _id
      const assetKey =
        m.assetId && typeof m.assetId === "object"
          ? (m.assetId._id?.toString() ?? String(m.assetId))
          : String(m.assetId ?? "");
      // centerId can be:
      //   - a populated Mongoose document (new per-center path) → has ._id
      //   - a plain MongoDB ObjectId (legacy path) → toString() gives hex string directly
      //   - a plain object from toObject() → has ._id
      let centerKey: string;
      if (!m.centerId) {
        centerKey = "none";
      } else if (typeof m.centerId === "object" && m.centerId._id) {
        centerKey = m.centerId._id.toString();
      } else {
        centerKey = m.centerId.toString();
      }
      const key = `${assetKey}|${centerKey}`;
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });

    console.log("📊 Total mappings before dedup:", formattedMappings.length);
    // Debug: show every entry's dedup key so we can spot duplicates
    formattedMappings.forEach((m, i) => {
      const assetKey =
        m.assetId && typeof m.assetId === "object"
          ...
          : String(m.assetId ?? "");
      let centerKey: string;
      if (!m.centerId) centerKey = "none";
      else if (typeof m.centerId === "object" && m.centerId._id)
        centerKey = m.centerId._id.toString();
      else centerKey = m.centerId.toString();
      console.log(
        `  [${i}] asset=${assetKey} center=${centerKey} centerName=${m.centerName}`,
      );
    });
...
export const updateMyAssetCounts = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const mappingProjectId =
      typeof mapping.projectId === "object"
    ...
    if (!mapping.centerId && targetCenterId) {
      console.log(
        `🔀 Legacy project-level mapping detected. Creating center-specific copy for center ${targetCenterId}`,
      );
      ...
      if (!centerMapping) {
        ...
        (mappingObj as any).centerId = new mongoose.Types.ObjectId(
          targetCenterId,
        );

        centerMapping = await CenterAssetMapping.create(mappingObj);
        console.log(
          `✅ Created new center-specific mapping: ${centerMapping._id}`,
        );
      }
    ...
    console.log(
      "🔍 [UPDATE] Current mapping - totalAssigned:",
    ...
    console.log(
      "💾 [SAVE] Before save - workingAsset:",
      mapping.workingAsset,
      "notWorkingAsset:",
      mapping.notWorkingAsset,
      "remark:",
      (mapping as any).remark,
    );
    await mapping.save();
    console.log(
      "✅ [SAVE] After save - workingAsset:",
      mapping.workingAsset,
      "notWorkingAsset:",
      mapping.notWorkingAsset,
    );
...
export const submitAudit = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const userCenters = Array.isArray(user.centers)
      ? user.centers
      : user.centers
        ? [user.centers]
        : [];
    const userProjectIds = userCenters.map((c: any) =>
      (c.projectId || c._id).toString(),
    );
    ...
    if (!mapping.centerId && targetCenterId) {
      console.log(
        "📋 submitAudit: Legacy mapping detected, creating center-specific copy for centerId:",
        targetCenterId,
      );
      ...
      if (existingCenterMapping) {
        ...
        console.log(
          "📋 submitAudit: Using existing center-specific mapping:",
          mapping._id,
        );
      } else {
        ...
        mapping = newMapping;
        console.log(
          "📋 submitAudit: Created new center-specific mapping:",
          mapping._id,
        );
      }
    }
