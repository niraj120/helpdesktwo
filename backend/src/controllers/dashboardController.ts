...
export const getDashboardStatistics = async (
  ...
  try {
      ...
      if (
        ...
        if (projectContext.accessibleProjectIds.length > 0) {
          baseQuery["metadata.projectId"] = {
            $in: projectContext.accessibleProjectIds.map(
    ...
    // Exclude merged secondary tickets from all dashboard counts
    baseQuery.isMerged = { $ne: true };
    ...
    const resolvedTickets =
      ticketsByStatus.find((s) => s.status === 4)?.count || 0;
    const pendingTickets =
      ticketsByStatus.find((s) => s.status === 2 || s.status === 3)?.count || 0; // In Progress or On Hold
