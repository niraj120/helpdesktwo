...
export const getPublicTable = async (
  ...
  try {
    ...
    if (table.dataSource === "articles") {
      ...
      // Use Promise.all for async URL refreshing
      const dynamicRows = await Promise.all(
        articles.map(async (article, index) => {
          const rowData: any = {};

          // Process columns sequentially to handle async URL refresh
          for (const column of table.columns) {
            ...
            for (const field of fieldMapping) {
              if (field === "pdfUrl" && article.pdfUrl) {
                ...
                value = await refreshSignedUrlIfNeeded(article.pdfUrl);
                break;
