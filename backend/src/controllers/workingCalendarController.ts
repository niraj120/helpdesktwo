...
// Get all working calendars (Super Admin only - no project filter)
...
export const getWorkingCalendarsByProject = async (req: Request, res: Response) => {
  try {
...
export const getDefaultWorkingCalendar = async (req: Request, res: Response) => {
  try {
    const { projectId } = req.params;
...
export const createWorkingCalendar = async (req: Request, res: Response) => {
  try {
    const { projectId, name, description, workingHours, holidays, timezone, isDefault } = req.body;
    ...
    const populatedCalendar = await WorkingCalendar.findById(calendar._id)
      .populate('projectId', 'name code');
...
export const updateWorkingCalendar = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, description, workingHours, holidays, timezone, isDefault } = req.body;
    const userId = (req as any).user?.id || (req as any).user?._id;
...
export const deleteWorkingCalendar = async (req: Request, res: Response) => {
  try {
    ...
    if (calendar.isDefault) {
      return res.status(400).json({
        success: false,
        message: 'Cannot delete the default working calendar. Please set another calendar as default first.',
      });
    }
