...
export const getDepartmentsByProject = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
  ...
  try {
    ...
    if (existing) {
      res
        ...
        .json({
          success: false,
          error: "A department with this name already exists in this project",
        });
      return;
    }
    ...
    if (error.code === 11000) {
      res
        ...
        .json({
          success: false,
          error: "Department name already exists in this project",
        });
      return;
    }
    res
