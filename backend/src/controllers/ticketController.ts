...
import fs from 'fs';
import { sendTicketCreatedEmail, sendStudentWelcomeEmail, sendTicketStatusChangedEmail, sendTicketClosedEmail } from '../utils/emailService';
import { sendTicketCreatedWhatsApp, sendTicketStatusChangedWhatsApp, sendTicketClosedWhatsApp } from '../utils/whatsappService';
import { sendTicketCreatedSMS, sendTicketStatusChangedSMS, sendTicketClosedSMS } from '../utils/smsService';
...
export const submitTicket = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Send WhatsApp notification to student (non-blocking - fire and forget)
    if (ticketData.Phone) {
      (async () => {
        try {
          await sendTicketCreatedWhatsApp(
            projectId,
            ticketData.Phone,
            {
              studentName: studentName,
              ticketNumber: ticket.ticketNumber,
              ticketSubject: ticket.subject || 'New Ticket',
              ticketStatus: getStatusName(ticket.status)
            }
          );
          console.log(`📱 WhatsApp ticket creation notification sent to ${ticketData.Phone}`);
        } catch (waError) {
      ...
      // Send SMS notification to student (non-blocking - fire and forget)
      (async () => {
        try {
          ...
          console.log(`📲 SMS ticket creation notification sent to ${ticketData.Phone}`);
        } catch (smsError) {
...
export const closeTicket = async (req: Request, res: Response) => {
  try {
    ...
    // Send WhatsApp notification for ticket closed (non-blocking - fire and forget)
    if (ticket.metadata?.studentPhone) {
      (async () => {
        try {
          await sendTicketClosedWhatsApp(
            ticket.metadata.projectId,
            ticket.metadata.studentPhone,
            {
              studentName: ticket.metadata.studentName || 'Student',
              ticketNumber: ticket.ticketNumber,
              resolution: 'Your ticket has been closed. Thank you for contacting us.'
            }
          );
          console.log(`📱 WhatsApp ticket close notification sent to ${ticket.metadata.studentPhone}`);
        } catch (waError) {
          console.error('Failed to send WhatsApp close notification:', waError);
          // Don't fail the request if WhatsApp fails
        }
      })();

      // Send SMS notification for ticket closed (non-blocking - fire and forget)
      (async () => {
        try {
          await sendTicketClosedSMS(
            ticket.metadata.projectId,
            ticket.metadata.studentPhone,
            {
              studentName: ticket.metadata.studentName || 'Student',
              ticketNumber: ticket.ticketNumber,
              resolution: 'Your ticket has been closed. Thank you.'
            }
          );
          console.log(`📲 SMS ticket close notification sent to ${ticket.metadata.studentPhone}`);
        } catch (smsError) {
          console.error('Failed to send SMS close notification:', smsError);
          // Don't fail the request if SMS fails
        }
      })();
      
      // Send Email notification for ticket closed (non-blocking - fire and forget)
      (async () => {
        try {
          await sendTicketClosedEmail(
            ...
            'Your ticket has been resolved and closed. Thank you for contacting us.',
            ticket.metadata.projectId,
            { studentName: ticket.metadata.studentName || 'Student' }
          );
          console.log(`📧 Email ticket close notification sent to ${ticket.metadata.studentEmail}`);
        } catch (emailError) {
          console.error('Failed to send email close notification:', emailError);
          // Don't fail the request if email fails
        }
      })();
    }
...
export const updateTicketStatus = async (req: Request, res: Response) => {
  try {
    ...
    // Send WhatsApp notification for status change (non-blocking - fire and forget)
    if (ticket.metadata?.studentPhone) {
      (async () => {
        try {
          await sendTicketStatusChangedWhatsApp(
            ticket.metadata.projectId,
            ticket.metadata.studentPhone,
            {
              studentName: ticket.metadata.studentName || 'Student',
              ticketNumber: ticket.ticketNumber,
              newStatus: getStatusName(statusNum),
              previousStatus: getStatusName(oldStatus)
            }
          );
          console.log(`📱 WhatsApp status change notification sent to ${ticket.metadata.studentPhone}`);
        } catch (waError) {
      ...
      // Send SMS notification for status change (non-blocking - fire and forget)
      (async () => {
        try {
          await sendTicketStatusChangedSMS(
            ticket.metadata.projectId,
            ticket.metadata.studentPhone,
            {
              studentName: ticket.metadata.studentName || 'Student',
          ...
          console.log(`📲 SMS status change notification sent to ${ticket.metadata.studentPhone}`);
        } catch (smsError) {
          console.error('Failed to send SMS status notification:', smsError);
          // Don't fail the request if SMS fails
        }
      })();
      
      // Send Email notification for status change (non-blocking - fire and forget)
      (async () => {
        try {
          await sendTicketStatusChangedEmail(
            ...
            ticket.ticketNumber,
            ticket.subject || 'Support Ticket',
            getStatusName(statusNum),
            getStatusName(oldStatus),
            ticket.metadata.projectId,
            { studentName: ticket.metadata.studentName || 'Student' }
          );
          console.log(`📧 Email status change notification sent to ${ticket.metadata.studentEmail}`);
        } catch (emailError) {
          console.error('Failed to send email status notification:', emailError);
          // Don't fail the request if email fails
        }
      })();
    }
