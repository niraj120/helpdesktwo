...
export const login = async (
  ...
  try {
    ...
    if (projectForJWT) {
      require2FA =
        projectForJWT.configuration?.securitySettings?.mfaRequired || false;
    }
    ...
    if (require2FA && userRoleName !== "Student") {
      ...
      console.log(
        `📱 2FA OTP generated and dispatched (masked) for ${email} (Phone: ${user.phone})`,
      );
    ...
    if (user.role) {
      if (typeof user.role === "object") {
        if ("name" in user.role) {
          ...
          console.log(
            "⚠️  Role object exists but no name field, keys:",
        ...
        try {
          ...
          permissions = await extractPermissionCodes(
            rolePermissions,
            `Login [${email}]`,
          );
          console.log(
            `✅ Extracted ${permissions.length} permissions from role.permissions array`,
          );
...
export const getMe = async (req: AuthRequest, res: Response) => {
  try {
    ...
    console.log(
      `🔑 getMe - User: ${user.email}, Permissions: ${permissionCodes.length}`,
    );
...
export const forgotPassword = async (
  ...
  try {
    ...
    // Store OTP using central otpStore (stores hashed value, optional Redis)
    const otpId = await otpStore.createOtp(email, otp, 10 * 60, {
...
export const resetPassword = async (
  ...
  try {
    ...
    const userForPolicy = await User.findOne({
      email: email.toLowerCase().trim(),
    });
    if (userForPolicy && (userForPolicy as any).projects?.length > 0) {
      const policyProject = await Project.findById(
        (userForPolicy as any).projects[0],
      );
      ...
      if (!policyResult.valid) {
        return res
          .status(400)
          .json({ success: false, error: policyResult.errors[0] });
      }
    }
