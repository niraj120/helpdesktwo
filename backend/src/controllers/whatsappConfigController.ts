...
export const updateWhatsAppTrigger = async (req: Request, res: Response) => {
    try {
        ...
        const validTriggers = [
            ...
            'ticketCreatedStudent', 'ticketCreatedAgent', 'ticketCreatedOnline',
            'ticketCreatedOffline', 'ticketCreatedEmail',
            'ticketAssigned', 'ticketEscalated', 'ticketReassigned',
            'ticketStatusChanged', 'ticketClosed', 'ticketRejected', 'ticketReopened',
            'ticketCommentAdded', 'ticketReplied',
            'ticketReminderAgent24hrs', 'ticketReminderAgent48hrs',
            'ticketDueSoon', 'ticketOverdue',
            'studentWelcome', 'studentOTP'
        ];
...
export const testWhatsAppTrigger = async (req: Request, res: Response) => {
    try {
        ...
        if (cleanPhone.length < 10 || cleanPhone.length > 15) {
            return res.status(400).json({
                success: false,
                message: 'Invalid phone number. Please include country code (e.g., 919876543210)'
            });
        }
        ...
        if (!config) {
            return res.status(404).json({
                success: false,
                message: 'WhatsApp configuration not found. Please configure settings first.'
            });
        }
        ...
        if (!config.apiBaseUrl || !config.accessToken) {
            return res.status(400).json({
                success: false,
                message: 'WhatsApp API not configured. Please set API Base URL and Access Token first.'
            });
        }
        ...
        if (!trigger.numberId) {
            return res.status(400).json({
                success: false,
                message: 'Number ID not configured for this trigger. This is the pre-approved template ID from WhatsApp.'
            });
        }

        // Use Number ID as template name if template name is not provided
        const templateName = trigger.templateName || trigger.numberId;

        console.log(`📱 Using template name: "${templateName}" for Number ID: "${trigger.numberId}"`);
        ...
        if (result.success) {
            return res.status(200).json({
                ...
                details: {
                    ...
                    sampleButtonParameters: sampleParams.buttonParams?.map(p => p.text) || []
                }
            });
        } else {
            return res.status(400).json({
