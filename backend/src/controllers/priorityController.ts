...
export const getPriorityById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const priority = await Priority.findById(id).populate(
      "projectId",
      "name code",
    );
  ...
  try {
    ...
    if (existing) {
      return res.status(400).json({
        success: false,
        message:
          "Priority with this name or code already exists for this project",
      });
    }
  ...
  try {
    ...
    if (
      ...
      const existing = await Priority.findOne({
        ...
        $or: [{ name }, { code: code?.toUpperCase() || priority.code }],
      });
    ...
    priority.isDefault =
      isDefault !== undefined ? isDefault : priority.isDefault;
    priority.description =
      description !== undefined ? description : priority.description;
