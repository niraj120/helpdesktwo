 ...
 */
export const getEmailCommunications = async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 * @access  Private (TICKET_VIEW_ALL or TICKET_VIEW_OWN)
 */
export const getEmailCommunicationById = async (req: Request, res: Response): Promise<void> => {
  try {
 ...
 */
export const sendTicketReply = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id: ticketId } = req.params;
    ...
    // Get agent info
    const agentName = user.fullName || `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'Support Agent';
    ...
    const lastMessageId = inReplyToMessageId || 
      (ticket.metadata as any)?.emailMessageId || 
      ticket.sourceEmailMessageId || 
      (previousEmails.length > 0 ? previousEmails[previousEmails.length - 1].messageId : undefined);
    ...
    console.log(`📧 Email service returned: fromEmail=${emailResult.fromEmail}, fromName=${emailResult.fromName}`);
    console.log(`👤 Agent fallback: agentEmail=${agentEmail}, agentName=${agentName}`);

    // Log outgoing email communication
    const emailCommunication = await logOutgoingEmail(ticket._id.toString(), {
      messageId: emailResult.messageId || `<temp-${Date.now()}@sac-helpdesk.com>`,
 ...
 */
export const getAllIncomingEmails = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (req.query.ticketNumber) {
      ...
      const ticket = await Ticket.findOne({ ticketNumber: req.query.ticketNumber });
      if (ticket) {
