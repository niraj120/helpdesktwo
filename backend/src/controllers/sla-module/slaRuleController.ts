  ...
  try {
    ...
    if (!isSuperAdmin) {
      if (callerRole?.projects?.length > 0) {
        ...
        // Non-super-admin with no projects assigned → show nothing
...
// Get single SLA rule
export const getSLARuleById = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const updateSLARule = async (
  ...
  try {
    ...
    const slaRule = await SLARule.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    })
      .populate("projectIds", "name code")
...
// Delete SLA rule
export const deleteSLARule = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const toggleSLARuleStatus = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
