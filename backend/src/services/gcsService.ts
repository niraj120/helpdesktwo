...
try {
  ...
  if (process.env.GCS_CREDENTIALS) {
    ...
    storage = new Storage({
      projectId: process.env.GCS_PROJECT_ID || credentials.project_id || 'helpdesk-dev-478611',
  ...
  // Otherwise, check for key file
    ...
    if (fs.existsSync(gcsKeyFile)) {
      ...
      console.log(`✅ GCS Storage initialized for Knowledge Base (from file: ${gcsKeyFile})`);
    } else {
      console.log('📁 Using local storage for Knowledge Base');
      console.log(`   Reason: GCS key file not found at: ${gcsKeyFile}`);
      console.log('   Set GCS_KEY_FILE env variable or create backend/config/gcs-key.json');
...
export class GCSService {
  ...
  static async uploadPDF(
    ...
    projectCode: string
  ): Promise<{ url: string; filename: string; size: number; fullPath: string }> {
    if (!bucket) {
      throw new Error('Google Cloud Storage is not configured. Please set GCS_PROJECT_ID, GCS_BUCKET_NAME, and GCS_KEY_FILE or GCS_CREDENTIALS environment variables.');
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      blobStream.on('finish', async () => {
        try {
          // Generate a signed URL with maximum allowed expiry (7 days - GCS limit)
          const [signedUrl] = await blob.getSignedUrl({
            version: 'v4',
            action: 'read',
            expires: Date.now() + 7 * 24 * 60 * 60 * 1000, // 7 days (maximum allowed by GCS)
          });
          
          console.log(`☁️ GCS: Uploaded PDF to ${fullPath}`);
          console.log(`🔗 Signed URL generated (valid for 7 days)`);
          resolve({
            url: signedUrl,
            filename: filename,
            fullPath: fullPath,
  ...
  static async deletePDF(filePathOrFilename: string): Promise<void> {
    try {
      ...
      const fullPath = filePathOrFilename.includes('/') ? filePathOrFilename : `kb-pdfs/${filePathOrFilename}`;
      await bucket.file(fullPath).delete();
      console.log(`✅ Successfully deleted PDF from GCS: ${fullPath}`);
    } catch (error) {
      console.error('Delete error:', error);
      throw new Error('Failed to delete PDF from GCS. Please check GCS configuration.');
    }
  }
   ...
   * Supports both old flat structure and new project folder structure
   */
  static async getSignedUrl(filePathOrFilename: string, expiresInMinutes: number = 60): Promise<string> {
    try {
      if (!bucket) {
        throw new Error('Google Cloud Storage is not configured. Signed URLs require GCS.');
      }
      ...
      throw new Error('Failed to generate signed URL. Please check GCS configuration.');
    }
  }
   ...
   */
  static async uploadRoleDocument(
    file: Express.Multer.File
  ): Promise<{ url: string; filename: string; size: number; fullPath: string }> {
    if (!bucket) {
      throw new Error('Google Cloud Storage is not configured. Please set GCS_PROJECT_ID, GCS_BUCKET_NAME, and GCS_KEY_FILE or GCS_CREDENTIALS environment variables.');
    ...
    return new Promise((resolve, reject) => {
      blobStream.on('error', (err) => {
        ...
        reject(new Error('Failed to upload role document to cloud storage. Please check GCS configuration.'));
      });
  ...
  static async deleteRoleDocument(fullPath: string): Promise<void> {
    try {
      ...
      await bucket.file(fullPath).delete();
      console.log(`✅ Successfully deleted role document from GCS: ${fullPath}`);
    } catch (error) {
   ...
   */
  static async uploadEditorImage(file: Express.Multer.File, projectCode: string = 'shared'): Promise<string> {
    if (!bucket) {
      throw new Error('Google Cloud Storage is not configured. Please set GCS_PROJECT_ID, GCS_BUCKET_NAME, and GCS_KEY_FILE or GCS_CREDENTIALS environment variables.');
    }
  ...
  static async deleteEditorImage(filePathOrFilename: string): Promise<void> {
    try {
      ...
      const fullPath = filePathOrFilename.includes('/') ? filePathOrFilename : `kb-images/${filePathOrFilename}`;
      await bucket.file(fullPath).delete();
  ...
  static async deleteProjectBrandingImage(url: string): Promise<void> {
    try {
      ...
      await bucket.file(fullPath).delete();
      console.log(`✅ Successfully deleted branding image from GCS: ${fullPath}`);
    } catch (error) {
  ...
  static async getFileMetadata(filePathOrFilename: string): Promise<any> {
    try {
      if (!bucket) {
        throw new Error('Google Cloud Storage is not configured. Cannot retrieve file metadata.');
      }
      ...
      throw new Error('Failed to get file metadata. Please check GCS configuration.');
    }
  }
}
