      ...
      if (
        ...
        return {
          sendMail: async (options: any) => {
            ...
            const fmtMsgId = (id: string) => id.startsWith("<") ? id : `<${id}>`;

            // Use the Message-ID already generated by sendTicketReplyEmail (e.g. <ticket-HSTR-...-reply-...@sac-helpdesk.com>)
            // so the same ID gets stored in TicketEmailCommunication and can be matched later.
            const outboundMessageId: string = options.messageId || `<graph-reply-${Date.now()}@sac-helpdesk.com>`;
            ...
            if (options.inReplyTo) {
              internetMessageHeaders.push({ name: "In-Reply-To", value: fmtMsgId(options.inReplyTo) });
            }
            if (options.references) {
              const refs: string[] = Array.isArray(options.references)
                ? options.references
                : [options.references];
              if (refs.length > 0) {
