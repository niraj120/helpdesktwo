...
import WhatsAppConfig, { IWhatsAppConfig, IWhatsAppTrigger } from '../models/WhatsAppConfig';
 ...
 */
export const TRIGGER_TEMPLATE_VARIABLES: Record<string, { name: string; description: string }[]> = {
  accountCreated: [
  ...
  ticketCreatedStudent: [
    { name: 'studentName', description: 'Student\'s full name' },
    { name: 'ticketNumber', description: 'Ticket number (e.g., MHCET-2025-0001)' },
  ...
  studentOTP: [
    { name: 'otpCode', description: '6-digit OTP code' },
  ],
};
...
export const sendWhatsAppTemplateMessage = async (
  ...
  try {
    ...
    if (!config || !config.apiBaseUrl || !config.accessToken) {
      ...
      console.log(`📱 [SIMULATED] WhatsApp message to ${phoneNumber} using template: ${templateName}`);
      return { success: true };
    }
    ...
    if (!numberId) {
      ...
      return { success: false, error: 'Number ID not configured for this template' };
    }
    ...
    const apiUrl = `${config.apiBaseUrl}/${numberId}/messages`;
    ...
    if (!messageId) {
      const errorMsg = 'WhatsApp API returned success but no message ID. Check your template name, parameters, and WhatsApp Business Account setup.';
    ...
    console.log(`✅ WhatsApp message sent to ${phoneNumber}, WAMID: ${messageId}`);
    ...
    console.error(`❌ WhatsApp message failed to ${phoneNumber}: ${errorMessage}`);
 ...
 */
export const getSampleParametersForTrigger = (triggerName: string): {
  bodyParams: TemplateParameter[];
  buttonParams: TemplateParameter[] | null;
} => {
  const sampleData: Record<string, { body: string[]; button?: string[] }> = {
    accountCreated: { body: ['John Doe', 'john@example.com', 'https://helpdesk.example.com/login'] },
    passwordReset: { body: ['John Doe', 'https://helpdesk.example.com/reset/abc123'] },
    ticketCreatedStudent: { body: ['John Doe', 'TICKET-2025-0001', 'Login Issue', 'Open'] },
    ticketCreatedAgent: { body: ['Agent Smith', 'TICKET-2025-0001', 'John Doe', 'Login Issue', 'High'] },
    ticketCreatedOnline: { body: ['John Doe', 'TICKET-2025-0001', 'Payment Issue'] },
    ticketCreatedOffline: { body: ['John Doe', 'TICKET-2025-0001', 'Document Request'] },
    ticketCreatedEmail: { body: ['John Doe', 'TICKET-2025-0001', 'Email Subject'] },
    ticketAssigned: { body: ['Agent Smith', 'TICKET-2025-0001', 'Login Issue', 'John Doe'] },
    ticketEscalated: { body: ['Manager Jones', 'TICKET-2025-0001', 'Senior Support', 'SLA Breach'] },
    ticketReassigned: { body: ['Agent Johnson', 'TICKET-2025-0001', 'Login Issue', 'Agent Smith'] },
    ticketStatusChanged: { body: ['John Doe', 'TICKET-2025-0001', 'In Progress', 'Open'] },
    ticketClosed: { body: ['John Doe', 'TICKET-2025-0001', 'Issue resolved successfully'] },
    ticketRejected: { body: ['John Doe', 'TICKET-2025-0001', 'Duplicate ticket'] },
    ticketReopened: { body: ['Agent Smith', 'TICKET-2025-0001', 'Customer reported recurring issue'] },
    ticketCommentAdded: { body: ['John Doe', 'TICKET-2025-0001', 'Agent Smith'] },
    ticketReplied: { body: ['Agent Smith', 'TICKET-2025-0001', 'John Doe'] },
    ticketReminderAgent24hrs: { body: ['Agent Smith', 'TICKET-2025-0001', 'Login Issue', '24'] },
    ticketReminderAgent48hrs: { body: ['Agent Smith', 'TICKET-2025-0001', 'Login Issue', '48'] },
  ...
  return {
    ...
    buttonParams: data.button ? data.button.map(text => ({ type: 'text', text })) : null
  };
};
...
export const sendTriggerMessage = async (
  ...
  try {
    ...
    if (!config.enabled) {
      console.log(`⚠️  [WHATSAPP SERVICE] WhatsApp sending is disabled for project ${projectId}`);
      // Log as blocked
      await logWhatsAppMessage({
        projectId,
        recipient: phoneNumber,
        templateName: (config.triggers as any)[triggerName]?.templateName || 'unknown',
        status: 'blocked',
