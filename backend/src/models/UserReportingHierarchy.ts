...
export interface IUserReportingHierarchyModel extends Model<IUserReportingHierarchy> {
  getDirectReportees(supervisorUserId: string | mongoose.Types.ObjectId, projectId?: string): Promise<any[]>;
  getAllReporteesRecursive(supervisorUserId: string | mongoose.Types.ObjectId, projectId?: string, maxDepth?: number): Promise<any[]>;
  getSupervisorChain(reporteeUserId: string | mongoose.Types.ObjectId, projectId?: string): Promise<mongoose.Types.ObjectId[]>;
  checkCircularDependency(supervisorUserId: mongoose.Types.ObjectId, reporteeUserId: mongoose.Types.ObjectId, projectId?: mongoose.Types.ObjectId): Promise<boolean>;
}

const UserReportingHierarchySchema = new Schema<IUserReportingHierarchy>(
  {
    reporteeUserId: { // MATCHES ACTUAL DB FIELD (the employee/reportee)
      type: Schema.Types.ObjectId,
      ref: 'User',
      required: [true, 'Reportee User ID is required'],
      index: true,
    },
    supervisorUserId: { // MATCHES ACTUAL DB FIELD (the manager/supervisor)
      type: Schema.Types.ObjectId,
      ref: 'User',
...
UserReportingHierarchySchema.statics.getDirectReportees = async function (
  ...
  const managerId = typeof supervisorId === 'string' 
...
UserReportingHierarchySchema.statics.getAllReporteesRecursive = async function (
  ...
  console.log(`📋 [HIERARCHY] Reportee IDs:`, allReportees.map(r => r.userId?.toString()));
...
UserReportingHierarchySchema.statics.checkCircularDependency = async function (
  ...
  const supervisorChain = await UserReportingHierarchyModel.getSupervisorChain(managerId, projectId);
  return supervisorChain.some((id: mongoose.Types.ObjectId) => id.equals(employeeId));
};

export const UserReportingHierarchy = mongoose.model<IUserReportingHierarchy, IUserReportingHierarchyModel>(
  'UserReportingHierarchy',
  UserReportingHierarchySchema
);
