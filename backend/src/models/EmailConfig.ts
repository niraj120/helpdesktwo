...
export interface IEmailConfig extends Document {
  ...
  // Delivery provider: 'smtp' (direct) or 'sendgrid' (API partner)
  emailProvider: "smtp" | "sendgrid";

  // SendGrid configuration (used when emailProvider === 'sendgrid')
...
emailConfigSchema.statics.findWithDecryptedPassword = async function (
  query: any,
) {
  ...
  if (config && config.smtpPassword) {
    try {
      ...
      console.error(
        "Failed to decrypt password in findWithDecryptedPassword:",
        error,
      );
      return config;
    }
