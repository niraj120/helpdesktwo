...
export interface IEscalationMatrix extends Document {
  ...
  getLevelByNumber(levelNumber: number, priorityCode?: string): IEscalationLevel | undefined;
  getNextLevel(currentLevelNumber: number, priorityCode?: string): IEscalationLevel | undefined;
  isTransitionAllowed(currentLevelNumber: number, targetLevelNumber: number, priorityCode?: string): { allowed: boolean; reason?: string };
}
...
EscalationMatrixSchema.pre('save', function(next) {
  ...
  if (this.priorityMode === 'SAME_FOR_ALL' && this.levels && this.levels.length > 0) {
    this.levels.sort((a, b) => a.levelNumber - b.levelNumber);
  }
  
  // Sort levels in each priority config in PER_PRIORITY mode
  if (this.priorityMode === 'PER_PRIORITY' && this.priorityConfigs && this.priorityConfigs.length > 0) {
...
EscalationMatrixSchema.methods.getLevelsForPriority = function(priorityCode: string): IEscalationLevel[] {
  if (this.priorityMode === 'SAME_FOR_ALL') {
    return this.levels || [];
  } else {
    const config = this.priorityConfigs.find((c: IPriorityConfig) => c.priorityCode === priorityCode.toUpperCase());
...
EscalationMatrixSchema.virtual('orderedLevels').get(function() {
  if (this.priorityMode === 'SAME_FOR_ALL') {
    return [...(this.levels || [])].sort((a, b) => a.levelNumber - b.levelNumber);
  } else {
    ...
    return this.priorityConfigs && this.priorityConfigs.length > 0
      ? [...(this.priorityConfigs[0].levels || [])].sort((a, b) => a.levelNumber - b.levelNumber)
      : [];
  }
});

// Instance method to get level by number for a specific priority
EscalationMatrixSchema.methods.getLevelByNumber = function(levelNumber: number, priorityCode?: string): IEscalationLevel | undefined {
  const levels = priorityCode ? this.getLevelsForPriority(priorityCode) : (this.levels || []);
  return levels.find((level: IEscalationLevel) => level.levelNumber === levelNumber);
};

// Instance method to get next level for a specific priority
EscalationMatrixSchema.methods.getNextLevel = function(currentLevelNumber: number, priorityCode?: string): IEscalationLevel | undefined {
  const levels = priorityCode ? this.getLevelsForPriority(priorityCode) : (this.levels || []);
  const sortedLevels = [...levels].sort((a, b) => a.levelNumber - b.levelNumber);
  return sortedLevels.find((level: IEscalationLevel) => level.levelNumber > currentLevelNumber && level.isActive);
};
...
EscalationMatrixSchema.methods.isTransitionAllowed = function(
  ...
  const targetLevel = levels.find((l: IEscalationLevel) => l.levelNumber === targetLevelNumber);
  if (!targetLevel) {
  ...
  if (this.escalationMode === 'SEQUENTIAL') {
    ...
    const currentIndex = sortedLevels.findIndex((l: IEscalationLevel) => l.levelNumber === currentLevelNumber);
    const targetIndex = sortedLevels.findIndex((l: IEscalationLevel) => l.levelNumber === targetLevelNumber);
    
    if (currentIndex === -1) {
      // Current level not found - allow escalation to first level
      return targetIndex === 0 
        ? { allowed: true }
        : { allowed: false, reason: 'Must start at the first escalation level' };
    }
    
    if (targetIndex !== currentIndex + 1) {
      return { allowed: false, reason: 'Sequential mode only allows escalation to the immediate next level' };
    }
  ...
  if (targetLevelNumber > currentLevelNumber) {
    ...
    if (!this.allowSkipLevel && targetLevelNumber > currentLevelNumber + 1) {
      return { allowed: false, reason: 'Skip level is not allowed in this matrix' };
    }
    return { allowed: true };
  }
  ...
  if (targetLevelNumber < currentLevelNumber) {
    ...
    if (!this.allowBackward) {
      return { allowed: false, reason: 'Backward escalation is not allowed in this matrix' };
    }
    return { allowed: true };
  }
...
export const EscalationMatrix = mongoose.model<IEscalationMatrix>('EscalationMatrix', EscalationMatrixSchema, 'escalationmatrixes');
